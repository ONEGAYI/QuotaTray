...
- 模型与定价目录：价格数据随 `data/pricing/v1/catalog.json` 独立更新（人工审核合并即发布），客户端定期自动接收新目录，无需升级应用；已下架（retired）模型保留最后已知价格
...
quota pricing catalog status   # 查看定价目录来源/版本/上次检查
quota pricing catalog update   # 手动拉取定价目录更新
...
- 官方平台价格（DeepSeek 等）随版本内置种子，并支持目录数据独立更新（改价格不发版，见 [定价目录维护指南](docs/定价目录维护指南.md)）；条目可字段级自定义（留空即回退预置）
