...
- **数据/日志目录快捷入口（2026-09-18 登记，桌面专属）**：设置页数据管理
  ...
  文案分层）。若未来提供移动端等价能力（如 SAF 定位数据目录），须单独
  设计并在真机验收后改写本条目；未实现前缺口保留。
- **迁移导入双模模态（2026-09-18 登记，桌面与 Android 共用 UI 已接线）**：
  ...
  机制免费提供。桌面模拟器外未验：真机 SAF 选择 → inspect 信息卡 → 密码
  档双模导入 → 倒计时/勾选锁定触控实效 → 计数反馈全链验收归 T-18（#126），
  完成前缺口保持。
