...
- `--window` 三态（优先级从高到低，详见 history-spec §7.1）：`all` 全部窗口；
  精确键匹配（向后兼容自定义窗口名字面量）；类别别名 `5h`/`five_hour`/
  `five-hour`、`weekly`/`week` 按语义类别过滤。缺省按范围选粒度
  （24h→5h 类、7d/30d→周类），选中类别缺失回退全部（另一规范类别存在
  时提示；余额单窗静默回退）。全部窗口视图跨类别时按「5 小时窗口 /
  周窗口 / 其他窗口」分段表头渲染。
- 分页：默认每页 20 行（`--page-size` 1..=500 覆盖）；终端下默认交互翻页
  （空格/回车/→ 下一页、b/← 上一页、q/Esc 退出）；`--page N` 指定页码即非交互
  打印该页（与 `--json` 互斥）；管道（非终端）输出整表，翻页交由调用方。
- `--json`：输出原始点数组（`{id, name, range, window, points}`，不分页不
  聚合；`window` 为实际生效的过滤口径，未过滤为 null）。
- 退出码：id 不存在、历史库打开失败、页码超界 → 1；范围内无历史 → 提示并 0。
- `clear`：无 id 清全部、有 id 清单条目（id 须存在）；确认默认否，`--yes` 跳过。
- `config export` 默认携带全量历史行；`config import` 的历史接线随策略分叉
  （2026-09-18 起）：merge 按主键幂等合并进本机历史库（同条目 id 跨机器
  续线），overwrite 单事务清空重插（详见 config export / import 节）；
  v1 老包无历史 section，merge 导入不受影响。

### quota config export / import

```text
quota config export <路径> [--yes]
quota config import <路径> [--yes] [--strategy merge|overwrite]
```

- `export` 导出完整 `AppConfig`（供应商、凭据、定价、自定义模型库）到
  `.qtray-export` 私有认证容器，双档可选（2026-09-18，spec #119）：
  - 交互模式默认**密码档**——口令 ≥8 位、不回显输入两次，Argon2id 派生
    密钥加密且密钥不随包（备份可安全外存）；便捷档沿用一次性随机密钥
    随包（文件等同明文凭据，提示照旧）。
  - `--yes` 跳过全部交互时落**便捷档**（既有脚本行为完全兼容），成功后
    仍向 stderr 追加便捷档安全警示。
- `import` 双模可选（`--strategy`，默认 merge）：
  - `merge`（合并）：条目按 id 并集、同 id 冲突以本机为准；自定义模型
    库 custom_models 按键并集（同键同 id 本机为准）；比较组合按
    (provider_id, window_key) 并集（上限 4 条）；历史沿用幂等合并——
    不丢本机任何数据。
  - `overwrite`（覆盖）：完全变成备份——配置与比较组合整体替换，历史
    单事务清空后重插备份行；v1 老包未携带历史字段（`None`）时不清本机
    历史（格式缺失 ≠ 断言历史为空），明确携带空历史（`Some([])`）的包
    照常清空（跨端口径见 AGENTS.md「导入双模」，审查 #127 对齐）。
  - 密码档包交互不回显输入一次口令；`--yes` 跳过风险确认但**不跳过
    口令输入**。错密码/损坏包报确定性错误「备份密码错误或包已损坏」。
- 口令永不作为命令行参数（不进 shell 历史与进程列表），仅经终端掩码
  交互读取；v1/v2 旧包继续可导入。
- 两个命令成功返回 0，文件/格式/Vault 错误返回 1；取消交互（含口令
  输入中止）返回 0 且不写文件。
...
- 迁移口令同口径（2026-09-18 起）：仅终端掩码交互读取，不提供任何明文
  口令命令行参数；口令不进日志、错误信息与 `--json` 输出。
