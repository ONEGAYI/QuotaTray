...
APK `QuotaTray_<版本>_android-arm64.apk`，由长期密钥签名，同签名版本可直接覆盖
升级；PR 的 `android-preview` job 仍上传 `QuotaTray-android-arm64-preview-debug`
...
`keyAlias`/`keyPassword` 四键，路径用正斜杠；keystore 与口令即密钥材料，该目录
