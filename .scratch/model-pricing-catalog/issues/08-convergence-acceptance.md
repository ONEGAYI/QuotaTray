...
状态：验收问题已修复，本地自动检查通过（2026-09-10）；实际设备与正式数据变更验收仍见「待人工」
...
本节优先于上方初次实施记录中的已完成勾选；源码复审与单元测试不能代替实际设备或正式发布验收。

- 缓存：全部信封写入统一锁内重读，旧实例网络失败/无变化不能覆盖他进程新目录；恢复历史成功检查时间。
  新增 `stale_instance_attempt_preserves_other_process_newer_catalog`、
  `failed_attempt_respects_foreign_write_lock`、`restarted_sync_preserves_last_success_after_failure` 回归测试。
- 托盘：价格和峰谷翻转判定都显式消费有效目录；新增非种子价格与窗口测试。
- 前端：修复自定义模型撞官方订阅 ID 时的计费类型、编辑页 missing 模型预览回退；
  主窗常显已下架/价格未知。新增 ProviderCard SSR 测试，覆盖实际渲染。
- 调度：Android 加入持续前台周期检查，后台跳过，保留回前台补检；注入时钟验证六小时与前后台切换。
- CLI：真实二进制入口测试以本地模拟代理证明 JSON 定价及目录只读命令零隐式联网。
- 资料：GUI/CLI 透出来源和核验日期，未知日期明确显示；资料按钮不提交编辑表单，移动命中区至少 44×44px。
- 发布校验工作流补齐 Linux CLI 的 dbus 开发依赖，并纠正必需检查与路径过滤的说明。

本地验证：`RUST_LOG=info cargo test --workspace`、workspace all-targets Clippy、fmt；
前端 238 项测试、lint、build；NDK 27.2.12479018 下 Android all-targets 交叉 Clippy。
`RUST_LOG=info` 仅用于测试子进程，避免继承宿主 warn 级别令已有日志冒烟丢失 info 事件。
两轴复审确认原十项修复闭环；第二轮发现的来源按钮表单语义与触摸尺寸也已修复复核。
