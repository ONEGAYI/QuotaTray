# 更新日志

本项目所有显著变更记录于此文件。格式基于 [Keep a CHANGELOG](https://keepachangelog.com/zh-CN/1.1.0/)。

## [0.15.0] - 2026-09-26

本版本围绕提醒与统计双线增强：新增额度恢复提醒与「网络环境」设置页，使用统计定位线引入区间净消耗与消耗/恢复分解；同时统一百分比展示口径——条目可配置主度量偏好，展示与提醒层全面改用剩余百分比，低余额阈值随升级自动换算，存量用户提醒触发时机数值等价。

### 新功能

**额度恢复提醒（桌面 / Android）（#132）**

- 百分比额度从低额恢复至高额时发送系统通知并入消息中心：默认在剩余回升至 95% 时触发，门槛可调；持续达标只提醒一次，回落清除登记后下次达标才重提，避免后台轮询每周期重复打扰
- 恢复判定跨重启持久化（`alert_state.json` 读改写增量合并）：后台无论通知开关是否开启均判定并落盘待展示消息，通知开关只拦截系统弹出；错过的恢复在下次打开时补展示

**「网络环境」设置页（桌面 / Android）（#133）**

- 设置新增「网络环境」页，代理主机与端口配置迁入该页集中管理；更新页保留可点击的指路入口

**模型与价格目录自动更新状态（桌面 / Android）（#134）**

- 目录状态行补全并新增自动更新口径小字：标明目录按每 6 小时周期自动检查更新，来源与时效一目了然

**定位线净消耗与消耗/恢复分解（桌面 / Android）（#135）**

- 定位线读数新增区间净消耗主数值（负值表示净恢复），可展开消耗/恢复分解：已观测累计消耗、累计恢复与断档期未观测净变化三项恒满足「消耗 − 恢复 + 未观测 = 端点净值」；桌面悬停展开、移动端触控展开
- 曲线值方向显式化：剩余量为常态方向，仅提供已用量的模板曲线按已用量解释，跨样本口径漂移自动过滤

**主度量展示偏好与剩余口径统一（桌面 / Android）（spec #137）**

- 条目级主度量偏好（自动 / 剩余百分比 / 剩余额度，缺省自动）：摘要位与提醒按偏好取数，数据形状与展示解耦
- 展示与提醒层百分比统一翻转为剩余口径（「已用 N%」→「剩余 N%」）；协议层数据保持已用语义不变
- 低余额阈值字段自动迁移（旧值按 100−换算、幂等、可回滚）：旧「已用 80% 触发」等价变为「剩余 20% 触发」，升级后提醒时机数值不变；新安装默认剩余 20%
- 统计曲线派生由单选改双产：同一窗口的金额曲线与百分比曲线独立成组并存

### Bug 修复

**定位线速率与峰值读数方向（桌面 / Android）（#135）**

- 对仅提供已用量的模板曲线，定位线消耗速率与峰值消耗读数此前按剩余量方向计算且未补偿，消耗被误标为回升；现统一按「剩余量下降 / 已用量上升 = 消耗」换算，两种口径读数方向一致

## [0.14.1] - 2026-09-21

本版本为桌面交互修补：统计页聚焦组合浮层的悬停收起加入 240ms 宽限，鼠标下移进层点选组合不再被提前收起。

### Bug 修复

**桌面聚焦浮层悬停收起宽限（16eef31）**

- 0.14.0 悬停驱动下鼠标离开药丸触发区即被立即收起——鼠标从药丸下移、尚未进入浮层时就消失，无法点选组合；现改为移出后延迟 240ms 收起，期间回到触发钮或浮层即取消收起
- 键盘路径不受宽限影响：Tab 聚焦展开、Enter/Space 开合照常，显式展开会作废挂起的收起倒计时；Esc、失焦、删光组合自动收起与移动端点击开合行为不变

## [0.14.0] - 2026-09-19

本版本重构配置迁移链路为「双档双模」：导出新增密码档（口令派生密钥不随包，迁移包可安全外存），导入新增合并并集模式，GUI 迁移交互换用自研模态窗并以三重防线拦截覆盖导入，CLI 同步支持 `--strategy`；设置页新增数据与日志目录快捷入口。同时按所有者修订将桌面聚焦组合开合改回悬停驱动，并修复安卓统计卡头窄屏竖排与迁移链多项健壮性问题（#127）。

### 新功能

**迁移包双档双模（桌面 / Android / CLI）（#127）**

- 导出双档：新增密码档——备份口令经 Argon2id 派生密钥加密（容器 v3），派生密钥不写入包内任何字节，迁移包可安全保存到网盘等不可信位置；便捷档维持一次性迁移密钥随包的现状语义；16 MiB 上限两档一致，v1/v2 旧包持续可导入
- 导入双模：合并模只补缺、不丢本机任何东西（条目按 id 并集且同 id 本机为准、自定义模型库并集、历史幂等合并、比较组合并集写回）；覆盖模完全变成备份（配置整体替换、历史单事务清空重插）；交互默认合并，脚本与旧调用路径缺省覆盖（维持既有行为）
- 导入前只读识别：GUI 导入模态展示容器版本与档位，不解密、无需口令
- GUI 导出/导入改自研模态窗（移动端自动全屏）：导出默认推荐密码档（≥8 位口令 + 二次输入校验），便捷档保留「等同明文凭据」完整警示；导入选覆盖展开三重防线——警示卡 + 5 秒阅读倒计时 + 风险勾选，确认钮转危险色；错误（含密码错误）就地展示不关弹窗，完成后反馈条目与组合的新增/跳过计数
- CLI：`config export` 交互选档（默认密码档，口令掩码输入两次，永不进命令行参数）；`config import` 新增 `--strategy merge|overwrite`（默认 merge）；`--yes` 按脚本兼容口径落便捷档
- v1 老包（未携带历史字段）覆盖导入不再清空本机历史；备份明确断言空历史时照常清空

**设置页数据与日志目录快捷入口（桌面）（#127）**

- 设置页「数据管理」分区新增「打开数据目录」「打开日志目录」：按运行模式在资源管理器打开实际目录（安装版 `~/.quotatray` 与其 `logs/`、便携版程序旁 `Data/` 与 `Data/logs/`），备份与排查不再手动拼路径；Android 无对应能力，入口不渲染

### Bug 修复

**安卓统计卡头窄屏竖排（#131）**

- 使用统计卡头移动端覆盖改换行布局：窄屏放不下时按钮组自然换到次行，标题不再被压缩成逐字竖排

**桌面聚焦组合开合改回悬停驱动（ec1e2b5）**

- 按所有者 2026-09-19 修订，推翻 0.13.2「点击开合为唯一路径」口径：桌面浮层改为悬停/Tab 聚焦展开、移出/失焦收起，键盘 Enter/Space 触发开合，Esc 收起兜底；移动端保持点击开合模态窗不变

**迁移链健壮性（#129）**

- 密码档派生与加解密改异步执行，导入导出期间 UI 不再冻结（上限内恶意 KDF 参数组合可致分钟级无响应的问题一并消除）
- 误选超大文件先做元数据预检报错，不再整读进内存后才提示超出上限；Android SAF 读取按 16 MiB 上限截断
- 导出口令与导入选项的调试输出脱敏，口令不进任何日志
- GUI 导入重选新文件时清空旧文件信息与口令，修复「错误横幅指向新文件、确认钮却提交旧文件」的错位提交；识别期间锁定关闭路径，防迟到回填造成状态残留
- GUI 内迁移降级告警改写入滚动日志（GUI 无 stderr 可见面）

## [0.13.2] - 2026-09-17

本版本按所有者裁决统一聚焦组合的入口与开合交互：药丸入口全平台一致、点击开合为唯一路径，移动端聚焦列表升级为居中模态窗，并收拢统计页工具栏的窄屏布局（#118）。

### 新功能

**移动端聚焦组合模态窗（#118）**

- 移动端「聚焦组合」由横向标签改为居中模态窗：行内单击聚焦/失焦、行尾两段确认删除，与桌面浮层共用同一套行组件样式
- 返回键、Esc、遮罩点击、关闭钮均可关闭模态窗；组合删光后自动收起
- 删除组合失败不再静默，错误提示直接显示在模态窗内

**统计页工具栏窄屏收拢（#118）**

- 窄屏下「添加组合」与时间尺度并入同一行（时间尺度靠右），聚焦药丸独占次行，三行并为两行

### 其他改进

- 聚焦组合入口全平台统一为药丸渲染，点击开合为唯一开合路径：桌面悬停展开/收起退役，浮层改为点击浮层外部或 Esc 关闭

## [0.13.1] - 2026-09-16

本版本重构使用统计页的组合管理与定位线交互：组合列表收进「聚焦组合」悬停浮层并以卡头药丸实时显示聚焦状态，定位线按钮改为三态语义、清空入口移至卡头，同时修复触屏拖动定位线期间的并发写入问题。

### 新功能

**聚焦组合浮层与状态药丸（桌面 / Android）（3a4d3ae）**

- 移除独立「管理」弹窗：组合列表收进「聚焦组合」悬停浮层，行内单击聚焦/失焦，行尾两段确认删除（首击图标变垃圾桶，再击才删除）
- 单位冲突被隐藏或已失效的组合保留灰色占位行，仍可定位与删除，不再占死 4 条上限
- 「聚焦组合」按钮升级为台阶分层药丸：右端下陷平台以小字 `FOCUSING` 与组合名、最新值实时显示聚焦状态，未聚焦显示「无聚焦组合」
- 移动端组合标签同步获得行内删除入口与两段确认

**定位线按钮三态与卡头清空（桌面 / Android）（3a4d3ae）**

- 「定位线」按钮改为三态：放置模式中点击退出；未满两条点击进入放置直接补位；满两条再点清空两条并从头定位
- 一键清空垃圾桶移入图表卡头（「定位线」与「重置视图」之间），仅清除不进入放置，无定位线时禁用；读数行保留逐条 × 删除

### Bug 修复

**触屏双指下的定位线并发写入（3524bfe）**

- 拖动定位线手柄期间封堵其余全部写入入口（一键清空、放置、逐条删除），修复第二指操作后松手落盘把刚删除或清空的定位线整字段写回的问题

### 其他改进

- 卡头「定位线」「清除全部定位线」提示换用应用标准悬停气泡：长提示限宽自动换行，不再出现系统气泡单行超长出窗；聚焦浮层 Esc 关闭后可用 Enter 或空格重开（3524bfe）
- 月牙状态平台的凹弧遮罩补 `-webkit-mask-image` 兜底旧 WebView，底色不再随遮罩解析失败丢失（3524bfe）

## [0.13.0] - 2026-09-15

本版本重构编辑弹窗的信息架构：脚本查询获得与模板一致的二级子页，字段按重要性重排（凭据优先、定价沉底），必填字段收纳进统一卡片，并修复代码编辑器不跟随暗色主题的问题。

### 新功能

**编辑弹窗分栏与字段重排（桌面 / Android）（#114）**

- 脚本查询形态新增二级子页：「运营商与模型」（名称、baseUrl、控制台直达、定价、凭据）与「查询脚本」（代码编辑、校验/试查），与模板形态同构；baseUrl 输入框移入「运营商与模型」子页
- 三个主 tab 的字段按重要性统一重排：API key 等凭据提到 baseUrl 之后，控制台直达地址在凭据下方，峰谷定价移至表单最底部
- 必填字段（名称、baseUrl 或平台选择、主/第二凭据）收纳进统一的卡片容器，与普通字段醒目分组
- 保存失败自动跳回对应子页：名称/控制台地址校验失败回「运营商与模型」，脚本校验失败回「查询脚本」

### Bug 修复

**代码编辑器暗色主题（桌面 / Android）（#114）**

- 模板 JSON 与脚本 JS 两处代码编辑器跟随应用明暗主题，暗色模式下不再呈白底

### 其他改进

- 脚本页「允许非 https 端点（allowInsecure）」开关内联至「校验/试查/AI 调试」操作行，不再单独占行（#114）

## [0.12.1] - 2026-09-10

本版本改进托盘图标的额度数字可读性（细圆环大字按目标像素直接栅格化），并将定价目录的 revision 编号口径定为日期式版本号（仅维护约定，不影响客户端判据）。

### Bug 修复

**托盘额度数字可读性（桌面端托盘）**

- 托盘图标采用已验收的细圆环搭配大数字方案：按目标像素尺寸直接栅格化嵌入的 Noto Sans Bold 数字子集，消除固定 32px 小字缩放造成的模糊（#113）
- 百分比省略百分号、余额从千位起紧凑缩写，悬停详情保留完整单位与精度（#113）
- Windows 按托盘所在屏幕的 DPI 缩放选择图标尺寸；分层配色、无数据空环与告警红点行为不变（#113）

### 其他改进

- 定价目录 revision 编号方案定为日期式版本号 `YYYYMMDDXYZ`（发布日 8 位 + 当日三位序号，首个日期式值 20260914001）：唯一硬性要求仍为「新值严格大于当前值」，校验器判据不变，存量 revision 1/2 无需迁移（f294c4a）

## [0.12.0] - 2026-09-10

本版本交付模型与定价目录独立更新链：价格与模型变更自此只需合并人工审核的数据包即可发布，客户端自动接收，不再要求升级应用版本；首发数据包（revision 2）已同步 DeepSeek V4.1-Flash 发布当日的新模型阵容。

### 新功能

**模型与定价目录独立更新链（桌面 / Android / CLI）（#107）**

- `data/pricing/v1/catalog.json` 成为预置定价单一数据源：构建时嵌入应用作离线种子，人工审核数据 PR 合并 main 后经 raw.githubusercontent 分发；数据与代码解耦，改价不再发应用版本
- 客户端目录同步：直连优先、代理兜底的串行双通道（同更新检测口径）；revision 单调不降级、坏包不落盘、跨进程锁防 CLI 与 GUI 互覆（#107）
- 桌面端 6 小时自动检查、失败 30 分钟退避，回前台补检；设置页可看目录状态与最近核验时间、手动检查更新（#107）
- CLI 新增 `pricing catalog status` / `pricing catalog update`（非 JSON 模式附带 5 秒预算的启动补检）（#107）
- 模型生命周期语义：下架模型保留最后已知价格并标注「已下架」；显式选择的缺失模型显示价格未知，绝不借用默认模型价格（#107）
- 卡片与编辑页可展开「官方模型资料」查看来源 URL 与人工核验日期；CLI `pricing show` / `model list --json` 同样透出（#107）
- CI 新增 catalog-data 工作流：数据 PR 自动执行结构校验、revision 递增与物理删除检查，与本地校验器同一 core 解析器（#107）

**首发目录数据包：DeepSeek V4.1-Flash 阵容（revision 2，#112）**

- DeepSeek 2026-09-10 发布 V4.1-Flash（模型名 `deepseek-flash`）并下线 V4-Flash / V4-Flash-Vision-Exp（旧模型名保留兼容路由、按 Flash 价计费）：目录当日收录——Flash 展示名更新、USD 档核实为官方新价（$0.006/$0.30/$1.20 高峰、空闲减半），0.11.0 因官方英文页故障遗留的 USD 旧值就此闭环（#112）
- Vision Exp 转为已下架状态，保留其真实生效期最后价格供历史花费估算（#112）
- V4 Pro 价格不变；官方预告 09-14 起其请求路由至 V4.1-Flash 计费，届时以后续数据包跟进（#112）

### 其他改进

- 定价抓取脚本适配 DeepSeek 新版两列定价页：表头脚注清理、恢复英文页 USD 候选产出（0.11.0 断供遗留），离线契约测试扩至 45 项（#112）
- 数据维护流程补全量测试门禁：种子字面断言散布三端测试，数据 PR 须跑 `cargo test --workspace`（#112）

## [0.11.0] - 2026-09-09

本版本将更新检测改为直连优先、代理兜底的串行双通道（匿名 GitHub API 按出口 IP 限额，代理共享额度易耗尽），并刷新预置定价：DeepSeek Flash 系列按 9·10 调价更新、智谱/Z.ai 新增 GLM-5.3-Flash 模型档、国际站随官网撤除 GLM-5-Turbo；预置价格自此经仓库内确定性抓取脚本核实维护。

### 新功能

**更新检测双通道——直连优先、代理兜底（桌面 / Android / CLI）**

- 检测改为串行双通道：直连成功（含无 release / 已最新）即用且不发代理请求；直连任何失败（网络 / 超时 / 限流 / 解析）且配置了代理时经代理重试一次，代理结果为最终结果；未配置代理维持单通道现状（#105）
- 动机：匿名 GitHub API 按 IP 限额（60 次/小时），代理共享出口的额度易被耗尽遭 403；配代理时直连通道以 `no_proxy` 真直连，防 Clash 等常开的系统代理静默劫持直连通道（#105）
- 下载继续经代理（口径不变）；Android 端共享同一检测链路，直连可达时不再依赖代理（#105）

**预置定价刷新（DeepSeek / 智谱 / Z.ai）**

- DeepSeek Flash 系列降价（2026-09-10 12:00 生效）：CNY 高峰档命中 / 未命中 / 输出更新为 0.04/2/8 元、空闲减半；Vision Exp 与 Flash 历史同价、随 Flash 系列更新；V4 Pro 不在降价范围（#106）
- 智谱国内站与 Z.ai 国际站预置新增 GLM-5.3-Flash 模型档（国内 0.23/0.8/2.8 元、国际 $0.03/0.15/0.50），取促销前原价档——官网 5 折限时促销 09-09 到期，预置以长期档位为准（#106）
- 国际站撤除 GLM-5-Turbo：docs.z.ai 定价页已无此模型；国内站保留（输入长度阶梯价取 <32K 基础档口径不变）（#106）

### 其他改进

- 新增仓库工具 `scripts/fetch_pricing/`：官网定价确定性抓取脚本（DeepSeek 中文页 SSG / 智谱 app.js 静态数据 / Z.ai SSG 三通道），一键全抓输出结构化价格矩阵供人工核对，35 项离线契约测试固化页面快照；预置价格自此经脚本核实维护（#106）
- DeepSeek USD 档因官方英文定价页路由故障暂维持旧值，待官方修复后核实同步（#106）

## [0.10.2] - 2026-09-07

本版本修复 Android 端应用内更新安装链的授权死路与定位线无法放入空采集区域两项问题，并为定位线读数行补充平均消耗速率；Android 更新安装自本版本起可在系统设置授权后直达安装确认页。

### 新功能

**定位线读数行平均消耗速率（桌面与 Android）**

- 聚焦单条曲线且两条定位线齐备时，时间差段内追加平均消耗速率（两点剩余量之差 ÷ 时间差小时数）：百分比曲线显示 `%/h`、余额曲线显示 `单位/h`（单位为空时仅 `/h`），最多 2 位小数去尾零，负值表示区间内回升（仅配 used 的模板曲线值为已用量、方向相反）（#102）
- 任一定位线附近无样本或时间差不足 1 分钟时不显示速率（无测量意义），与时间差文案「至少 1 分钟」口径对齐（#102）

### Bug 修复

**Android 应用内更新安装链授权（真机端测实证）**

- Android 包声明 `REQUEST_INSTALL_PACKAGES`（所有者重新确认，推翻原「不声明」口径）：此前系统「安装未知应用」授权列表搜不到本应用、授权页开关置灰，应用内点「安装」被系统弹回，只能走文件管理器兜底；声明后授权列表可见本应用，授权完成即可一步拉起系统安装确认页（#103）
- 更新页提示行同步改为「授权为主出路、文件管理器为兜底」；未来 Google Play 构建须剥离该权限（分发通道分流条款）（#103）

**定位线空采集区域不可放置**

- 点击图表经坐标换算得到的时刻带小数毫秒：吸附命中时取整数样本时刻侥幸可用，落入空采集区域（不吸附）时浮点时刻使后端持久化反序列化失败、保存回退，表现为定位线放不上——Android 端因数据稀疏、空采集区域多而最先暴露，桌面端共用同一逻辑（#104）
- 未吸附时刻统一归整为整数毫秒（放置与拖动共用的唯一时刻出口），空采集区域可正常放置并持久化；模拟器全链实证含修复前后对照（#104）

## [0.10.1] - 2026-09-05

本版本集中修复 CLI 凭据读取的误报与中断问题：读取失败不再一律误报「未找到」而是按类型透出真实原因，并新增进程内凭据快照，游戏反作弊等软件拦截凭据文件的窗口期内查询照常进行、无需退出游戏。

### Bug 修复

**CLI 凭据读取错误分类与拦截窗口期快照回退（Codex / Claude / Grok / Gemini）**

- 真机实证：鸣潮（腾讯 ACE 反作弊）等游戏在运行期间经内核驱动全局拒绝读取 `~/.codex/auth.json` 等本机 CLI 凭据文件（文件 ACL 无变化，游戏退出自动恢复），QuotaTray 定时刷新撞上拦截窗口即报错；且原实现把所有读取错误统一吞成「未找到」，真实原因被长期掩盖（#101）
- 读取错误按类型透出真实原因：文件缺失仍报安装引导（文案不变）；权限被拒明确提示「无权读取（安全软件/反作弊拦截或权限不足时会出现）」；共享冲突等其余 IO 错误携带原始系统错误码（#101）
- 新增进程内凭据快照：读得到文件就更新快照，被拦截时回退旧快照继续查询——游戏运行期间 Codex 等卡片保持正常刷新（token 有效期为天级，远长于拦截窗口）；冷启动即遇拦截（无快照可用）时如实报错（#101）
- 修复悬停展开卡片下边界的 hover 自持振荡（#99）

### 其他改进

- 修复 release 构建下 logging 模块的两条编译警告（#98）
- 前端样式规范补登 T-014 卡片 hover 反馈几何中性契约段（#100）

## [0.10.0] - 2026-09-05

本版本为使用统计比较图新增「定位线」测量工具：在图表上放置最多两条竖直定位线即可读取两点时刻与时间差，聚焦单条曲线时同步显示该曲线在定位线时刻的值；功能经三轮验收反馈与两轮子代理审查全量收口后合入。

### 新功能

**使用统计定位线（桌面与 Android）**

- 图表卡头新增「定位线」按钮：进入放置模式后点击图表放置竖直定位线，自动吸附最近数据点；最多两条、满两条自动退出，Esc 或再次点击按钮随时退出（#97）
- 图表上方读数行常驻显示各线时刻，两条时额外显示时间差（如「2天3小时15分」）；每条线可单独删除，右端垃圾桶图标一键清空；读数行按满载内容预留高度，放置、删除与聚焦切换不引起卡片高度跳变（#97）
- 定位线顶端手柄可沿时间轴拖动微调，与另一条重合时原地不动；聚焦单条曲线时每条定位线旁显示该曲线最近样本值（与悬浮读数同口径，无样本显示「—」）（#97）
- 定位线持久化到 settings.json（仅本机，不随配置迁移包导出）；超出当前视图的定位线不画线，读数行以弱化样式与悬停说明提示（#97）

### 其他改进

- 移动端定位线手柄命中区满足 44px 触摸规范，取值由 mobile-style 契约测试锁定（#97）
- 前端样式规范新增 T-013 定位线契约段与 DT-001 图表定位线令牌 `--qt-chart-marker`（明暗成对）（#97）

## [0.9.0] - 2026-09-04

本版本为代理与弱网环境下的查询稳定性建立「自动止血 + 事后定位」闭环：查询瞬时失败自动重试、网络错误透出底层根因，并新增常驻端滚动 JSONL 诊断日志。两项主变更均经双角度子代理审查与交叉 CI 门禁全量收口后合入。

### 新功能

**滚动 JSONL 诊断日志（桌面、便携与 Android）**

- 常驻端新增滚动 JSONL 日志：按天 + 单文件 5MB 双条件滚动、保留 7 天，逐行合法 JSON 可直接被 jq 等工具消费；目录跟随数据根（安装版 `~/.quotatray/logs/`、便携版 `Data/logs/`、Android 应用私有目录），`RUST_LOG` 环境变量可临时调深级别（#95）
- 查询链路与引擎生命周期事件打点：query_done（条目/通道/错误分类/耗时）、query_retry、engine_rebuild（触发源与新旧代理配置）、proxy_reconcile、startup（版本/运行形态/数据目录），代理故障的时间线从此有留痕锚点（#95）
- Android WorkManager 后台刷新冷启动路径同样落日志（此前 stderr 无处可去），与前台路径进程级幂等并存（#95）
- CLI `quota query --watch` 落盘同目录独立基名日志（与 GUI 互不干扰），补上清屏重绘吞掉历史错误轮次的留痕缺口（#95）

**查询瞬时失败自动重试**

- 超时、网络错误与 HTTP 408/429/5xx 在 2.5 秒后自动重试一次：代理软件切换节点、重载配置等秒级扰动不再表现为「断开、需手动开关条目代理才能恢复」；认证/解析类确定性错误保持零重试立即透出（#96）
- 桌面轮询、CLI（含 watch）、Android 前台与后台刷新经查询引擎单点全部生效（#96）

### Bug 修复

- 网络错误文案拼接完整底层原因链（如 `os error 10061` 连接被拒 / `10054` 连接被重置），经既有脱敏管道透出到卡片错误详情、CLI 输出与日志——故障层一眼可辨，不再只有顶层 `error sending request`（#96）
- 使用统计图表平滑按时间尺度门控：15 分钟及以下细粒度桶保留轻度高斯平滑，1 小时等粗粒度桶关闭平滑保留真实曲线，避免长周期视图失真为三角波；额度重置跳变与各类读数始终基于真实样本（#94）

### 其他改进

- 修复 android-preview 交叉编译门禁三处失败（Android cfg 半边的 clippy 盲区）（#95）
- file-tree 技能升级：目录条目写入防呆与 check 报错分流（#93）
- README 新增界面速览节并入库三张界面图

## [0.8.5] - 2026-08-31

本版本为使用统计带来多曲线比较与近 24 小时视图两项增强，并集中修复 Android 端多项界面与网络问题；多曲线比较经一轮独立分级审查（P0–P3）全量收口后合入。

### 新功能

**使用统计多曲线比较（桌面与 Android）**

- 使用统计页支持最多四条 Provider+模型/窗口组合同屏对照，百分比与同一绝对单位可共存；四色稳定色槽（明暗成对 CSS 令牌）随 settings.json 持久化，增删组合不漂移既有颜色（#92）
- 「添加/管理」组合使用居中 82dvh 模糊浮窗：添加时即时校验重复、上限与单位冲突，单位不兼容的组合以警示条显式计数而非静默隐藏；孤立单点曲线以端点圆渲染（#92）
- 组合随配置迁移包（v2 信封可选字段）携带，导入整体替换本机组合；归一化校验（去重、四条上限、色槽修复）收进 core 供 GUI/CLI 统一复用，常驻 GUI 保存设置时以磁盘为权威合并 CLI 专属字段，CLI 导入的组合不再被覆盖回退（#92）
- 桌面端聚焦单条曲线时显示多行气泡详情，且仅聚焦曲线恢复长期缺失区的灰色渐隐斜纹与断点标记（#92）
- Android 端全宽比较图表 + 图表下方常驻读数（aria-live），单指拖动更新游标，统计页工具栏按钮补齐 44px 触摸命中区（#92）

**使用统计近 24 小时视图**

- 统计页新增「近 24 小时」视图范围（15 分钟粒度），与既有 7 天档并列切换，快速查看日内消耗节奏（#90）

### Bug 修复

- Android 安装包 launcher 图标回落为 Tauri 默认图标：工程初始化链补 tauri icon 品牌化步骤（#87）
- Android 消息面板被卡片遮挡且左缘截断、代理端口在安卓指向自身：修复弹窗遮罩层级回归，代理主机 URL 预检收口全部穿透形态（#89）

### 其他改进

- 移动端更新页错误详情改为可点击的 disclosure 展开态，避免长文案挤占布局（#91）
- file-tree 技能升级至 mv/mv-batch 版本，支持受控移动重命名（#88）
- 规范与文档同步：前端规范新增 T-013 多曲线比较与 DT-004 系列色令牌登记，AGENTS.md 设计决策与术语表、历史存储规格、移动端能力缺口追踪同步更新（#92）

## [0.8.4] - 2026-08-30

本版本新增阿里云余额监控（百炼按量计费共享的账户级余额）：双凭据预置平台接入 BssOpenApi，编辑页内提供中英双语配置指引渲染器；Android 端已同批完成模拟器全链实证。

### 新功能

**阿里云余额监控（预置平台）**

- 新增预置平台「阿里云余额」：经阿里云 BssOpenApi `QueryAccountBalance` 查询账户级可用额度（百炼按量计费的扣费来源），支持负余额（欠费）如实展示（#84）
- 本平台为预置平台中首个双凭据形态：凭据一填 AccessKey ID、第二凭据填 AccessKey Secret，两者均 AES-GCM 加密落盘、编辑时留空即保持不变（#84、#85）
- 错误分类对齐双轨语义：RAM 未授权（引导授予 `AliyunBSSReadOnlyAccess` 只读策略）、AccessKey 有误等确定性错误即时透出；限流与服务端 5xx 归瞬时失败走保留旧值重试（#84）
- 卡片「访问控制台」直达阿里云费用与成本中心（usercenter2.aliyun.com）（#84、#86）

**配置指引渲染器（GUI，桌面与 Android）**

- 添加/编辑页内可打开平台配置指引：轻量 Markdown 渲染（标题/列表/粗体/行内代码/链接/引用块）以嵌套弹窗呈现，外链经系统浏览器打开（#85）
- 指引文档按语言分目录（`docs/guide/{zh,en}/`）随应用打包：跟随界面语言自动选档，缺失时回退另一语言；图片资产打包管线预留（`docs/assets/bundle/`）（#85、#86）
- 首批指引：阿里云余额监控配置指引（中英双语）——RAM 专用子账号创建、费用只读授权、AccessKey 保存与安全须知、常见问题（#85、#86）

### 其他改进

- 文档：百炼余额查询预研落档（Qwen OAuth 停摆与模型 key 无余额端点的探证、BssOpenApi 实测打通）（#84）
- Android 端（模拟器 API 36）全链实证回写移动端能力缺口追踪：真凭据查询、双语指引弹窗、嵌套返回键、外链拉起均通过，多厂商设备差异面留真机验收（#86）

## [0.8.3] - 2026-08-30

本版本补齐 Android 端消息与后台能力：消息中心移动入口、系统通知（运行时权限 + 前后台生命周期）与 WorkManager 后台周期刷新（低余额提醒常态化，默认关闭）；移动端触摸目标同步全量整改至 44px。

### 新功能

**Android 消息中心与系统通知（Preview）**

- 顶部应用栏新增消息常显入口：铃铛 + 未读红点 + 触屏下拉面板，与桌面消息中心同源；「发现新版本」消息卡片可一键直达设置·更新页（#78）
- 消息模型扩展至三类：更新就绪（桌面静默安装）、「发现新版本」（Android 更新检测出新版本时生成，会话内同版本防重）与低余额（两端共用：查询成功后按余额阈值百分比判定，多窗口条目取最高值）（#78）
- 系统通知（Android）：独立通知渠道 + Android 13+ 运行时权限流（设置页可请求授权或跳转系统设置），应用退后台时更新与低余额提醒改以系统通知送达（#79）
- 通知开关两端真实生效（默认开启，桌面现状行为不变；显式关闭后更新与低余额通知均拦截）（#79）
- 低余额边沿触发防重：首次达标才提醒、持续达标静默、余额回落清除登记后下次达标再次提醒，后台周期查询不会重复打扰（#79）
- 🧪 预览性质：消息中心与系统通知仍属 Android Preview（真实设备完整验收未完成，端测清单见 `docs/测试单/`），不构成稳定支持承诺

**Android 后台周期刷新（Preview，默认关闭）**

- 设置·通用新增「后台刷新」区块：默认关闭，开启后按所选周期（15–360 分钟，默认 30）在后台查询全部启用条目；系统省电策略下实际触发可能延后，不承诺精确分钟级（#80、#81）
- 后台查询结果只写入历史库：统计图表回前台即可见后台数据；不触碰前台内存与缓存，避免双写竞态；查询失败静默、下个周期自然重试（#80）
- 低余额提醒获得常态化消息源：应用未运行时 WorkManager 可拉起进程独立完成「查询 → 落库 → 低余额判定 → 系统通知」全链（冷启动自足），与前台共享同一防重登记，不产生重复通知（#80、#81）
- 🧪 预览性质：后台刷新属 Android Preview，多厂商省电策略下的实际触发节奏待真机验收，不构成稳定支持承诺

### 其他改进

- 移动端触摸目标全量整改至 ≥44px：设置页下拉、分段控件、模板预设与帮助折叠钮、峰谷周几切换与行内文字按钮、统计页下拉与重置钮等统一达标，桌面样式零改动（#77）
- 文档：移动端能力缺口追踪拆出独立文档（docs/移动端能力缺口追踪.md）持续维护，AGENTS.md 同步瘦身（#82）

## [0.8.2] - 2026-08-29

本版本交付 Android 端应用内更新链：应用内即可完成新版本检测、下载保存与系统安装器拉起，GitHub 渠道升级不再需要手动对比版本号与浏览器下载；同步补齐 CI 交叉 lint 门禁与真机端测清单。

### 新功能

**Android 端应用内更新链（Preview）**

- 应用内更新页（设置 → 更新）向 Android 开放：进入页面自动检测一次（距上次 ≥5 分钟）+ 手动「立即检查」，检测出 GitHub 新版本后可直接下载（#72、#74）
- 下载经系统文档选择器（SAF）保存：预填官方资产名 `QuotaTray_<版本>_android-arm64.apk`，进度条复用桌面同款展示；下载完成显示「安装」按钮（#74）
- 「安装」以系统安装器接管（ACTION_VIEW + 授权临时读权），全程不声明自安装权限 `REQUEST_INSTALL_PACKAGES`（Play 审核红线）（#74）
- 安装引导按系统版本分层：Android 7 直接弹系统确认页；8~15 授权「安装未知应用」后一步到位；Android 16+ 未声明权限时被系统闸口弹回（实证 AppOps 授权亦无效），提示行引导从文件管理器打开已保存 APK 安装（确定性出路），并保留授权设置页入口作为旧版本次出路（#74）
- core 资产选择器接入 Android APK 命名契约：同一 Release 含 WoA ARM64 zip 与 APK 时选择器只命中 APK（误匹配由契约测试锁定拒绝）（#72）
- 同版本重复检测不清空已下载的 APK；检测出新版本自动失效（版本快照比对）（#74）
- 🧪 预览性质：Android 仍为 Preview（真实设备完整验收未完成，端测清单见 `docs/测试单/`），不构成稳定支持承诺

### 其他改进

- CI：Android 工作流新增交叉 Clippy 门禁（aarch64-linux-android 目标，`-D warnings`），桌面/移动 cfg 分叉两侧同时受检；18 处既有分叉警告以 cfg 门禁显形边界后清零（#74）
- 文档：Android 16 真机验证记录与「更新与下载」预研报告（含「安装未知应用」闸口 API 36 实证）、安卓端真机端测清单（#73、#75）

## [0.8.1] - 2026-08-29

本版本落地 Android 端签名发布链：正式 Release 起附带固定密钥签名的 ARM64 APK，同签名版本可覆盖升级；桌面端自动更新检测改为应用运行期间定期轮询。同时修复两个影响 Android 构建的缺陷。

### 新功能

**Android 签名发布链（Preview）**

- 发布 tag 推送后 CI 自动构建固定密钥签名的 Release APK（`QuotaTray_<版本>_android-arm64.apk`）并附加到 Release 草稿；构建后强制断言产物非未签名变体、签名证书指纹与 keystore 一致、v2 签名方案存在、versionCode 与 tag 派生值一致，并以 aapt2 校验包名与 ABI（#69）
- 同签名版本可覆盖升级而无需卸载（已在模拟器验证 0.8.0 → 0.8.1 原地升级、应用数据保留；真机验收进行中）（#69）
- 本地构建签名 APK：`pnpm android:init` 后放置 `keystore.properties` 即可产出签名 Release APK，无该文件时退化为未签名构建（#69）
- 🧪 预览性质：Android 仍为 Preview（真实设备完整验收未完成），不构成稳定支持承诺（#69）

### Bug 修复

- Android 构建读取中文路径 keystore 时签名失败：`Properties.load` 固定按 ISO-8859-1 解码导致路径乱码，改用 UTF-8 Reader 读取（Tauri 官方模板同样受影响）（#69）
- Android versionCode 恒为 1 致升级链无版本区分：tauri 配置无 version 字段时 tauri-cli 跳过派生，构建脚本现经 `--config` 注入 workspace 版本，恢复 `major*1000000 + minor*1000 + patch` 原生派生（#69）

### 其他改进

**自动更新检测改为常驻轮询（桌面端）**

- 桌面端自动更新检测从「启动时 + 每日固定时刻（默认 09:00）+ 24h 节流」改为应用运行期间每 5 分钟后台轮询，新版本感知更及时；设置页移除「每日检测时刻」项，开关文案改为「应用运行期间定期在后台检查新版本」（#67，Close #66）
- 手动「立即检查」与检测失败同样落盘节流时间戳，不产生失败风暴；CLI 一次性进程维持 24h 节流语义不变（#67）
- 旧 `settings.json` 残留的 `update_check_time` 键自动兼容（未知字段容忍，契约测试锁定升级路径）（#65、#67）

- CI：Android Release 工作流设纯三段 tag 门禁（预发布后缀不进入构建）、concurrency 防同 tag 并发互撞、超时放宽至 60 分钟（#69）
- 文档：README 双语新增「Android（预览版）」下载小节，Android 端说明补本地签名构建指南与口令转义注意事项（#69）

## [0.8.0] - 2026-08-29

本版本交付桌面端完整自动更新闭环（可选自动下载、消息中心提醒、一键静默安装）与 Android 触摸优先预览端，新增 Windows on ARM（ARM64）预览资产与双端「访问控制台」直达入口。

### 新功能

**自动更新闭环（桌面端）**

- 检测到新版本后可自动后台下载安装包（设置页「更新」新增自动下载开关，默认关闭；便携与 ARM64 预览形态保持手动覆盖引导）（#60）
- 标题栏新增消息中心：铃铛 + 未读红点 + 「更新就绪」消息卡片，卡片内「现在安装」直接进入静默安装（明示将自动重启，点击即确认）；主窗不可见时改发系统通知（#60）
- 安装改为 NSIS 静默模式：无安装向导、装完自动重启应用、全程无 UAC 弹窗（#60，Close #58）
- 应用重启后自动恢复「已下载待安装」状态（按资产名探测下载目录，免重复下载）；启动时自动清理不再需要的旧安装包，安装失败或选择稍后安装的文件会保留（#60）

**Android 预览端（Preview）**

- 触摸优先移动界面：底部双导航（账户 / 使用统计）+ 顶部应用栏 + 全屏编辑与设置页，返回键逐层关闭页面（#61）
- 复用桌面核心能力：三类 Provider（预置 / 模板 / 脚本）增删改查、峰谷定价、历史统计、配置导入导出（系统文档选择器）、语言 / 主题 / 刷新间隔 / 代理设置（#61）
- 凭据保护同样落地：Android Keystore 保护主密钥、配置仅存 AES-GCM 密文，并关闭系统自动备份防止「密文恢复而密钥缺失」（#61）
- 依赖桌面 CLI 登录文件的订阅查询（Claude / Codex / Gemini / Grok）在移动端隐藏入口；桌面安装包更新链不适用于移动分发（#61）
- 🧪 预览性质：尚未完成真实 Android 设备验收（厂商 WebView、通知、下载、跨版本升级等），不构成稳定支持承诺（#61）

**控制台直达（双端）**

- 余额卡片新增「访问控制台」入口：一键用系统默认浏览器打开对应平台控制台（余额 / 充值 / 账单页），免手动找网址（#63，Close #59）
- 20 个预置平台内置官方控制台地址，条目级可自定义覆盖（编辑页「控制台地址」字段，留空回退预置）；桌面为图标按钮，Android 为「控制台」+ ↗ 图标的 trailing 文字按钮（44px 触摸命中区）（#63）
- 仅允许 http/https 链接（后端白名单校验），未配置地址的条目不渲染入口（#63）

**Windows on ARM（ARM64）预览资产**

- 新增 ARM64 普通预览包与便携预览包（`arm64-preview.zip` / `arm64-preview-portable.zip`），经交叉编译构建与产物架构校验（#57）
- ARM64 形态的更新检测引导下载 zip 后退出手动覆盖（#57）
- 🧪 预览性质：尚未在真实 Windows on ARM 设备验收，资产名与说明保留 Preview 声明（#57）

### 其他改进

- 开发者体验：新增 `pnpm desktop:dev` 开发入口，1420 端口被 WinNAT 动态圈占或程序占用时自动双栈探测顺延，并同步 Tauri devUrl（#62）

## [0.7.0] - 2026-08-28

本版本交付 Windows x64 便携版（解压即用、数据全部随身于 `Data/` 目录）与双端「清空全部数据」功能，并在 README 立起三种数据形态的保密级别全局对照；发布资产自此由一键脚本产出（NSIS 安装包 + 便携 zip）。

### 新功能

**Windows 便携版 x64（core / quota-cli / quota-desktop）**

- zip 解压到任意可写目录即可运行：配置、查询历史与便携主密钥全部保留在旁的 `Data/` 目录，不写注册表、无系统残留，删除目录即完成卸载；包内含 GUI（`QuotaTray.exe`）与 CLI（`quota.exe`），共用同一份数据（#56）
- 首次运行展示安全确认页：正文为两行核心警示（目录等同明文凭据、泄露需轮换全部 API Key），完整固定安全提示收进问号图标点击展开（含粗体与代码样式渲染），确认按钮 5 秒倒数解锁防误触；取消则不写入任何敏感文件并退出（#56）
- CLI 新增 `--portable` 参数并自动检测 exe 旁 `portable.marker`（与 `--config` 互斥）；便携首启同样要求交互确认，非交互环境未初始化时确定性失败并给出初始化指引（#56）
- 更新检测按运行形态精确匹配资产：安装版只找 setup 安装包、便携版只找 portable zip（完整文件名相等匹配，不跨架构、跨形态回退）；便携形态下载更新包后引导退出解压覆盖，不自动安装；设置页为便携形态时禁用开机自启（避免拔盘后残留无效启动项）（#56）
- 同机安装版与便携版互斥运行（GUI 二次启动自动聚焦现有窗口并给出提示）；`quota vault status` 兼容检查文件密钥后端的健康状态（#56）

**清空全部用户数据（quota-cli / quota-desktop）**

- 一键删除全部供应商条目、加密凭据密文、峰谷定价与自定义模型库、查询历史与余额快照缓存，保留应用偏好（settings）与主密钥——转让设备、排障重来的就地清理手段（#56）
- GUI 入口位于「设置 → 数据管理」：二级确认弹窗 5 秒倒数解锁，执行期间弹窗完全锁定；清空后主界面、托盘圆环与悬停面板即时进入空态（#56）
- CLI 为 `quota clear`：交互确认默认否，`--yes` 跳过；非交互会话未显式 `--yes` 时确定性拒绝（退出码 1）（#56）

**其他**

- 设置新增「数据管理」页签，聚合配置跨机器迁移与清空全部数据两项入口（#56）
- 设置更新页与 `quota --version` 显示运行平台（安装版/便携版）（#56）

### 其他改进

- 发布资产一键打包：仓库根 `package.cmd` 产出 NSIS 安装包与便携 zip，含版本单源解析、包内 GUI/CLI 的 PE 架构断言与中英双份包内说明（固定安全提示与 README 逐字一致，契约测试锁定）（#56）
- README（中英）新增三种数据形态（安装版/便携版/迁移包）的保密级别全局对照表，补充清空配置的安全语义（不构成安全擦除承诺）；修正设置入口名称为「数据管理」（#56）

## [0.6.1] - 2026-08-27

本补丁版本主体交付供应商卡片拖拽排序，主窗卡片列表可拖动换位并同步托盘图标与 CLI 列表顺序；同时修复硅基流动国内站接口停服的报错语义、使用统计悬停卡片遮挡曲线等问题。

### 新功能

**供应商卡片拖拽排序（quota-desktop）**

- 卡片左缘竖条把手按下拖动即可换位：被拖卡片逐帧跟手，其余卡片按目标槽位带过冲曲线让位（高度参差也精确对位），松手速度计入落位时长、快甩滑行更久；`prefers-reduced-motion` 下动效退化为直接换位（#52）
- 拖拽防御与中断回弹：4px 激活阈值避免点按误触；ESC / 窗口失焦 / 指针中断整体回弹不落库；拖拽中抑制悬停上浮等干扰动效并支持边缘自动滚动（#52）
- 键盘 ↑↓ / Home / End 调序并保持焦点连续；落库采用乐观更新即时生效、后端 300ms 合并写盘，落库前按位核对缓存序，列表被外部替换（如导入配置）时放弃写盘防止旧序覆盖（#52）
- 松手落库后同步托盘图标子菜单序与 CLI 列表序；托盘图标对「未指定图标条目」的回退取第一个启用条目，排序会改变该回退选择，属排序的预期语义（#52）

### Bug 修复

- 硅基流动国内站 `/v1/user/info` 已于 2026-08-14 停止服务（HTTP 410，替代 API 尚未发布）：国内站查询遇 410 改报确定性错误「接口已停止服务、不代表 API Key 无效」，不再与无效 Key 混淆；国际站保持通用 HTTP 错误路径。官方替代 API 发布后将移除该特判（#53，Close #50）
- 使用统计悬停数据点时详情卡片不再固定压在绘图区顶部遮挡贴顶的百分比曲线：改为跟随数据点锚定，点在上半区时卡片放下方、下半区翻上方（#48）
- 卡片错误文案过长被截断时，悬停即可查看完整报错（含已脱敏的错误详情），无需另行复制排查（#53）

### 其他改进

- 仓库新增分层 Git hooks 本地门禁：`pre-commit`（cargo fmt + 前端 lint，秒级）与 `pre-push`（clippy + tsc，分钟级），仓库根 `setup-hooks.cmd` 幂等配置（#49）
- file-tree 维护技能新增 `add-batch` / `rm-batch` 批量命令：整批原子生效、一份清单只占一步撤销历史，并统一 rel 落盘为正斜杠规范形态（#47）
- 新增 WoA（Windows on ARM）与便携版预研决策文档，固化 ARM64 Preview 发布口径与便携版密钥方案（#54）

## [0.6.0] - 2026-08-26

本版本面向 AI 辅助调试与 new-api 系中转站适配：模板/脚本新增第二凭据槽 `{{apiKey2}}`（令牌之外再携用户 ID 等双凭据形态），AI 调试求助升级为可直接交给本机 Agent 的完整任务提示词（Windows 安装包内置 quota CLI），assist 调试契约附完整示例并新增复用本机凭据的端测子命令。

### 新功能

**第二凭据槽 apiKey2（core / quota-cli / quota-desktop）**

- 模板与脚本支持 `{{apiKey2}}` 变量：new-api 系中转站余额查询需「系统访问令牌 + 用户 ID」双凭据（`Authorization` 与 `New-Api-User` 双头）。凭据经同一 vault 加密存储（空 = 保持不变、不回显）、迁移包转写、错误详情双槽脱敏；未配置第二槽时查询报确定性引导错误（#46）
- GUI 编辑弹窗（模板/脚本形态）新增「第二凭据」输入框，保存与试查全链路接通；CLI `set-key --slot 1|2` 写入对应槽，stdin 试查在草稿引用时按需交互收集（#46）

**AI 调试求助直连本机 Agent（quota-desktop）**

- 模板/脚本编辑器内联 AI 调试面板：脱敏求助提示词、`.qtray-assist.json` 诊断包保存、复制给本机 Agent 的完整任务提示词——含 `resolve_quota_cli_path` 解析的真实 CLI 绝对路径、PowerShell 命令、联网查证指引、安全边界（不得索要凭据、唯一允许的真实试查是 assist test）与输出契约；保存路径随输入变化失效并在复制前重写，Agent 永远拿到最新现场（#46）
- Windows 安装包内置 quota CLI：构建链暂存同 profile CLI 并映射为资源 `bin/quota.exe`，开发版与安装包中的 Agent 提示词都能给出真实可执行路径（#46）

**assist 调试契约与端测（quota-cli）**

- `assist schema` 内嵌模板/脚本完整示例（演示 `transforms` 顶层键与 `{"const": ...}` 字段形态、双凭据头写法），示例自身通过静态校验与离线解析的契约测试闭环——Agent 不再需要逆向 DSL；新增 `assistTest` 段落显式声明端测通道语义（#46）
- 新增 `assist test --mode <m> --input <诊断包> [--base-url]`：诊断包携带已保存条目的 `entryId`（本地标识符，非凭据），CLI 以包内草稿覆盖条目查询配置、复用本机两槽凭据密文走引擎真实查询一次；凭据不出 vault、不进输出，失败输出透出已脱敏的响应体片段（detail），退出码遵循三分约定（#46）

### Bug 修复

- 编辑供应商弹窗的全屏遮罩不再盖住自定义标题栏——此前弹窗打开期间无边框窗口无法拖动、最小化或关闭（#46）
- quota CLI 路径探测的 PATH 回退排除 System32/SysWOW64：Windows 自带同名 NTFS 配额工具不得被当作本软件 CLI 拼进 Agent 提示词（#46）
- 构建链修复：release 缺 CLI 仅在 Tauri CLI 驱动的构建中硬失败，CI 与普通 `cargo check --release` 不再被误伤；CLI 路径契约测试按平台拼接 EXE_SUFFIX，Linux 矩阵确定性成立（#46）

### 其他改进

- 诊断包脱敏清理覆盖 `api_key2` 键与 `New-Api-User` 头的自家形态，两槽占位符（含 Bearer 前缀组合）不被误伤；schema 脚本示例字段名与产物解析器对齐（snake_case），Agent 照抄不再静默丢字段（#46）
- 开发目录清理器纳管 CLI 暂存目录 `src-tauri/generated/`；脚本页签文案去除开发期里程碑标记（#46）

## [0.5.0] - 2026-08-26

本版本交付 M5 使用历史底座与走势查看：查询结果滚动保留 30 天的时序存储（SQLite）、CLI 走势命令与迁移包携带历史，桌面端新增使用统计页签并以趋势曲线展示剩余额度；同时为主题切换加入圆形揭示动效、收敛 Provider 刷新语义，亮色主色调调整为中性灰白。

### 新功能

**历史数据存储（core / quota-cli / quota-desktop）**

- 查询结果滚动保留 30 天（`~/.quotatray/history.db`，SQLite）：每条用量一行，重复写入幂等、同名多窗口自动 `#2/#3` 消歧、失效行跳过防假断崖；schema 经 `PRAGMA user_version` 版本化迁移，降级运行拒绝打开（#39）
- CLI 新增 `quota history` 命令组：`show` 查看余额/额度走势——24h/7d/30d 三档范围（桶粒度 15m/1h/6h），`--window` 窗口语义过滤（5h/周窗口分类，缺省按范围自动选粒度，选中类别缺失回退全部），跨类别分段表头、交互式分页与 `--json` 输出；`clear` 清空历史；`quota remove` 删除条目时同步清其历史（#39）
- `.qtray-export` 迁移容器升级 v2：随配置携带历史数值行，导入按主键幂等合并；v1 老包仍可导入，旧版程序读取 v2 拒绝。历史行仅数值字段不含凭据，16 MiB 容器上限超限时报错并附清库提示（#39）

**使用统计页签（quota-desktop）**

- 主窗口新增「账户 / 使用统计」页签：面板切换在最大模糊点替换内容后上浮恢复清晰，页签随鼠标距离显现靛青径向聚光（#41）
- 使用统计接入历史库趋势曲线：按条目与窗口选择查看剩余额度走势，时间轴常驻选定时间窗，悬停十字线查看采样点详情，缺失时段以间隔条纹标注（#42）

**主题切换动效（quota-desktop）**

- 主题切换时新主题从主题按钮位置圆形揭示扩散（View Transitions），跟随系统的实时变色同受覆盖（#40）

### Bug 修复

- 切换主题/语言等显示设置不再触发无关的 Provider 刷新与额外网络请求（#40）
- Provider 刷新语义收敛：只由首次取数、轮询到点、手动刷新、条目变更与配置导入触发；页签切换改为常挂载消除卸载重查，恢复聚焦/网络重连在轮询周期内不追加查询，单条目变更按条目失效不陪查，悬停托盘不再触发全量刷新（#41）

### 其他改进

- 亮色模式主色调由淡蓝调调整为中性灰白（#f8f8f8），与暗色主题风格对齐（#42）
- 仓库新增 file-tree 文件树维护技能：tree.json 单一数据源，脚本化渲染 AGENTS.md 文件树与标签词表，支持一致性校验与撤销重做（#41；渲染控制字段与 kind 标识随后升级，#42）

## [0.4.3] - 2026-08-25

本补丁集中修复托盘悬停状态失步、跨窗口 Provider 缓存不同步及删除确认框定位异常。

### Bug 修复

- 托盘悬停面板偶发在鼠标移开后仍不收起，手动关闭后首次重新悬停又无法弹出：增加真实光标看门狗，在托盘图标与面板之外持续 250ms 后主动隐藏；失步状态下将后续 `Move` 恢复为进入事件，使首次重新悬停即可显示（#37）
- 新增、编辑或删除 Provider 后，托盘面板的账户下拉列表仍显示旧缓存：后端广播 Provider 变更事件，主窗口与托盘面板分别失效 Provider 列表、查询状态和快照缓存（#37）
- 删除 Provider 卡片时，二次确认框受卡片悬停变换影响而居中于卡片：通用对话框改为挂载到页面根节点，恢复以主窗口为定位参照（#37）

## [0.4.2] - 2026-08-25

本版本预置平台从 12 家扩容至 20 家：新增 StepFun / Novita AI / MiniMax Coding Plan 三家按量与套餐平台、NewAPI 系中转站通用预置模板，以及 Claude / Codex（ChatGPT 订阅）/ Gemini / Grok 四家官方订阅用量查询——复用本机官方 CLI 的登录凭据，添加条目无需输入 key；同时配套条目级查询代理开关，chatgpt.com 等被墙站点可按条目走本机 HTTP 代理。

### 新功能

**订阅四家：Claude / Codex / Gemini / Grok（core / quota-cli / quota-desktop）**

- CLI 凭据复用路线：查询时只读本机官方 CLI 的登录文件（`~/.claude/.credentials.json`、`~/.codex/auth.json`、`~/.gemini/oauth_creds.json`、`~/.grok/auth.json`），QuotaTray 不做任何 OAuth 登录、凭据不落明文、用后即弃；添加条目跳过 key 输入，编辑对话框以提示卡说明"凭据来自本机 CLI"（#34 / #35）
- Claude 订阅：5 小时/周（含 Opus / Sonnet 独立周窗）多窗口用量百分比与重置倒计时，未知窗口 tier 自动兼容扫描（#34）
- Codex（ChatGPT 订阅）：5 小时/周/30 天多窗口（按窗口秒数自动标注），plan_type（plus / prolite / pro）透传；User-Agent 伪装 codex-cli 防 Cloudflare 拦截（#34，真机验证通过）
- Gemini Code Assist：OAuth token 过期自动刷新（gemini-cli 公开 client 凭据，不写回 CLI 文件），按 Pro / Flash / Flash Lite 分组取最紧额度与对应重置时间（#35）
- Grok 订阅：gRPC-web 私有端点，无公开 proto 契约下以 protobuf 启发式扫描提取额度百分比与重置时间（#35）
- 错误双轨：CLI 凭据文件缺失/未登录 → 确定性错误并引导先登录官方 CLI；网络抖动/被墙 → 瞬时错误（保留旧值）（#34 / #35）

**条目级查询代理（core / quota-cli / quota-desktop）**

- 供应商卡片新增代理开关（Globe 图标）：开启后该条目查询走设置中的网络代理端口，关闭走直连；原「更新代理端口」升级为「网络代理端口」，更新检测/下载/订阅查询共用（#34）
- 开启代理但未配置端口 → 确定性引导错误；设置中变更端口后查询引擎热重建，无需重启应用（#34）
- CLI `quota add` / `quota edit` 向导同步询问代理开关；`quota dev-smoke` 新增 `--proxy` 开关用于真机验证代理通道（#34 / #36）

**预置平台扩容（core）**

- 新增 StepFun（阶跃星辰）、Novita AI、MiniMax Coding Plan（国内/国际双站）三家预置平台，含品牌图标接线（#30）

**NewAPI 系通用预置模板（quota-desktop）**

- 模板编辑器预设库新增 NewAPI 系形态：约 40 家 one-api 系中转站复用同一查询结构（`GET {base}/api/user/self`），一键填入（#31）
- 模板条目按名称启发匹配接入 NewAPI 品牌图（#31）

### Bug 修复

- 添加供应商的平台选择器二级菜单在窄窗口溢出主窗、与右栏联动滚动：改为浮层菜单 + 双栏独立滚动（#32）
- main 分支 CI 全红：clippy 严格模式三处失败（grok 冗余 let-else 与恒等运算、add 向导函数参数数超限）及 GUI 冒烟示例缺新字段；修复并将 clippy 纳入提交前硬门禁（#36）

### 其他改进

- HTTP 响应新增字节保真通道（raw）：gRPC-web 二进制协议不再受 lossy UTF-8 转换损坏（#35）
- 预研报告：阿里百炼 API-Key 路线实测证伪留档（Blocked，社区端点全变体 404 + 网关仅认控制台 OAuth）；订阅四家批次勾选完成（#34 / #36）

## [0.4.1] - 2026-08-25

本版本收口桌面端两处体验断链：更新安装包下载后可直接在设置页一键安装（自动退出应用并启动安装向导），下载位置移至临时目录；添加供应商的「模板」形态重构为「运营商与模型 / 设置模板」二级子页，并附带预设模板库与模板编写说明。

### 新功能

**更新一键安装（quota-desktop）**

- 安装包下载完成后，设置页主按钮变为「立即安装」：确认后运行安装包并自动退出应用（NSIS 安装向导接管），替代此前"下载完无任何入口、需自行定位运行"的断链（#27）
- 下载位置从系统下载目录改为 `%TEMP%/QuotaTray/Downloads`（临时目录语义，随系统清理回收，丢失重下即可）（#27）
- 已下载状态由后端记录：重开设置页仍能直接安装；检测到不同版本安装包自动失效，安装包文件丢失时自动回退到可重下状态，网络故障不丢已下载记录（#27）
- 安全面：安装命令只运行后端自记录的路径；写入侧校验 release 资产名（纯文件名 + `.exe`，杜绝路径上跳/盘符/ADS 形态）、运行侧校验路径限于下载目录内，并对下载目录被预置 symlink/junction 的场景设防（#27）

**模板编辑器二级子页（quota-desktop）**

- 「模板」页签拆为两个子页：「运营商与模型」（名称、baseUrl、API key、峰谷计价）与「设置模板」（预设模板、JSON 编辑器、校验/试查、编写说明），原先挤在一页的高密度表单按职责分层；峰谷计价草稿在子页切换间保留（#28）
- 「设置模板」子页新增 5 形态预设模板一键填入：通用示例 / 单对象余额 / 站点可变 baseUrl / 总额已用 / 多窗口（与 `examples/templates` 同构）（#28）
- 预设点亮采用语义等价比较：serde 序列化往返补全的缺省键（`method: "GET"` 等）不算改动——「点预设 → 保存 → 重新编辑」仍能点亮；实质修改取值路径才视为脱离预设（#28）
- 新增「模板编写说明」折叠卡：`{{apiKey}}` / `{{baseUrl}}` 变量来源、取数字段速查、算术变换与多窗口写法、最小示例（#28）
- 保存时模板校验/解析失败自动切换到「设置模板」子页，错误现场直达（#28）

## [0.4.0] - 2026-08-25

本版本落地 JS 脚本查询：在预置平台与声明式模板之外新增第三类查询方式——QuickJS 沙箱内运行 `{request, extractor}` 脚本，覆盖模板表达不了的复杂平台（字段间运算、多窗口聚合、特殊签名），core / CLI / 桌面端三端全链打通；同时修复峰谷标签跨翻转边界停留在旧判定、one-api 系平铺错误体说明丢失两类缺陷。

### 新功能

**JS 脚本查询（core / quota-cli / quota-desktop）**

- 新增 script 条目类型：脚本内定义 `request()` 与 `extract(resp)` 两个全局函数，HTTP 由宿主执行、脚本只做纯计算；`{{apiKey}}` / `{{baseUrl}}` 变量在代码字符串层面注入，脚本作者接触不到真实凭据，脚本可安全分享（#24）
- 沙箱限制：内存 16MiB、栈 256KiB、单次执行 5 秒 CPU 上限，不注入任何宿主 API（无网络/文件系统），每次执行独立运行时；脚本异常可能回显的凭据在错误消息中统一打码（#24）
- 保存期干跑校验：假变量替换后执行脚本、验证双函数存在与 `request()` 产物形状（不发 HTTP），错误带行号定位（#24）
- CLI 新增 `quota script test`：`--entry` 复用已存条目密文试查；`--json` stdin 双形态宽容解析（脚本配置 JSON 或纯 JS 文本，`examples/scripts/*.js` 可直接重定向，形似配置却解析失败时附提示）（#25）
- `quota add` 向导新增「script」选项（粘贴代码 + 干跑校验重试 + 是否访问非 HTTPS 地址确认）；`quota edit` 支持脚本代码重粘贴与 allowInsecure 修改（#25）
- 桌面端编辑对话框 script 页签从预留态转为实态：CodeMirror JS 编辑器 + baseUrl + allowInsecure 开关（启用时明示风险）+ 校验/试查 + 内置默认示例（#26）
- 新增 `examples/scripts/` 可运行示例：`basic.js` 最小闭环、`multi-window.js` 多窗口与字段间运算（含协议说明与已知边界）（#25）

### Bug 修复

- 悬停面板/主窗卡片的峰谷标签跨过翻转边界（如 18:00）后停留在旧判定，而托盘已切换：后端每分钟检测到任一条目峰谷翻转时向全部窗口广播事件，前端判定改锚事件时刻，与托盘菜单保持一致（Close #15，#23）
- one-api 系少数端点的错误响应体为平铺字符串（`{"error":"..."}`），此前只认嵌套 `error.message`，错误说明被丢弃、只显示裸 HTTP 状态码：现嵌套形态之后回退平铺形态，非字符串形态不误提取（#22）
- CLI 多行粘贴在管道/重定向喂入时把文件内空行当结束符，含空行的 JS 代码与美化 JSON 后半段被静默丢弃：管道现读到 EOF；交互粘贴脚本代码改为单独一行 `.` 结束（空行照常保留），模板 JSON 保持空行结束（#25）

### 其他改进

- 依赖与兼容边界：引入 rquickjs 0.12（内嵌编译 QuickJS-NG），MSRV 提升至 1.87；旧版本二进制读取含 script 条目的配置会报未知类型，升级为单向（#24）

## [0.3.2] - 2026-08-24

本版本解决网络受限环境下的更新下载：设置页新增「更新代理端口」（CLI 共用同一设置），检测与下载安装包可经本机 HTTP 代理（如 Clash）完成；同时修复直连不可达时下载长时间零进度无提示、失败后状态仍显示「发现新版本」的静默问题。

### 新功能

**更新代理（core / quota-cli / quota-desktop）**

- 设置页「更新」分页新增更新代理端口输入框：填本机 HTTP 代理端口（如 Clash 的 7897），留空直连；越界输入自动收到 1-65535（#21）
- 检测更新与下载安装包共用该代理设置；CLI `quota update` 自动读取同一 settings.json，生效时输出提示行（#21）
- core 新增 `proxy_url_of`（端口 → `http://127.0.0.1:{port}` 的单一拼接口径）、`ReqwestAssetDownloader::try_with_proxy` 与 `ReqwestHttpClient::new_with_proxy`：显式代理不叠加环境变量代理，非法代理地址报错而非静默直连（#21）

### Bug 修复

- 安装包下载通道加 15 秒连接超时：直连不可达时快速失败，不再挂满 10 分钟总超时且期间零进度零提示（#21）
- 桌面端下载失败不再静默：操作错误优先于「发现新版本」徽章，失败立即以错误态呈现完整原因（#21）

## [0.3.1] - 2026-08-24

本版本聚焦查询失败的排查体验：余额查询失败（如「响应不是合法 JSON」）时保留经密钥脱敏的服务端响应详情，桌面端卡片一键复制完整报错，CLI `--json` 同步透出；报错文案中的密钥材料（含大小写变体、URL 参数、Bearer 回显等形态）自动打码。

### 新功能

**查询失败排查详情（core / quota-cli / quota-desktop）**

- 查询失败时保留脱敏后的服务端响应详情：JSON 解析失败附 serde 行列位置（说明"怎么不合法"）与响应体片段（说明"响应长什么样"），HTTP 错误附完整脱敏响应体（#20）
- 桌面端卡片错误行新增「复制报错信息」按钮（悬停提示），一键复制 `[分类] 文案 + 详情` 全文，复制结果经短时反馈提示；凭据未配置的业务失效不提供复制（#20）
- CLI `quota query --json` 错误对象新增可选 `detail` 字段（无详情时省略，不改变既有输出形状；表格输出不变）（#20）
- 安全设计：脱敏采用结构化正则（字段名保留、仅打码值）+ 本次请求真实密钥的字面量替换（Bearer 剥壳、URL query、模板自定义位置登记、URL-encoded 与稳定前缀形态），先清洗后截断至 2048 字符；详情仅在用户主动复制时进入剪贴板，不写入日志、托盘菜单、启动快照或 CLI 表格；报错文案通道（HTTP 错误说明、平台业务码、模板取值错误）同样全量脱敏（#20）

## [0.3.0] - 2026-08-24

本版本扩展平台覆盖并打通配置迁移：新增 Kimi Code 订阅用量与智谱/Z.ai 通用 API 按量余额两类预置查询；配置跨机器迁移从 core 安全基座一路接入 CLI 与桌面端；同时补齐更新下载进度展示、供应商聚合选单与平台官方头像。

### 新功能

**查询与平台（core）**

- Kimi Code 国内/国际订阅用量：解析 `/coding/v1/usages` 的 5 小时与每周额度窗口，RFC3339 重置时刻透传，余量本地推导；与 Kimi 开放平台余额分属独立凭据域，并补齐错误分类与订阅定价预置（#17）
- 智谱/Z.ai 通用 API 按量计费余额：新增 `zhipu_api`（CNY）/ `zai_api`（USD）预置平台，Bearer 认证直接计量；优先解析 credit grants，仅在端点明确 404/405 时回退 balance 形态；与 GLM Coding Plan 的裸 key、套餐窗口、订阅定价语义彻底分离，两站按量模型价格随版本预置（#18）
- 配置跨机器迁移基座：私有二进制容器（魔数 + 版本 + 长度 + AES-GCM 认证）；每次导出生成一次性迁移密钥逐条转写凭据，机器主密钥永不写入迁移包；导入时重加密到目标机器 Vault，任一步失败不产生部分结果；附格式篡改、跨机器转写与失败原子性契约测试（#16）

**命令行（quota-cli）**

- 新增 `quota config export <path>` / `quota config import <path>`：导出前提示迁移包含一次性密钥、敏感级别等同明文凭据，导入前提示整体替换当前配置；`--yes` 供自动化脚本跳过确认；帮助与结果文案全量双语（#19）

**桌面端（quota-desktop）**

- 设置对话框新增「数据迁移」页：经系统文件选择器导出/导入 `.qtray-export` 迁移包（带本地时间的默认文件名，省略扩展名自动补齐）；导出与导入均为双重风险确认；导入成功后清空旧结果与快照、重建托盘并跨窗口广播刷新（#19）
- 添加/编辑供应商改为品牌聚合双栏选单：一级按平台分组展示官方 SVG，二级展开国内/国际与余额/订阅选项，支持悬停、方向键与 Enter/Escape 操作；未知预置平台保留兜底分组（#17）
- 预置平台官方头像：DeepSeek、SiliconFlow、OpenRouter、Kimi、智谱、Z.ai 六组品牌 SVG 覆盖全部预置平台，未知平台回退首字母（#17）
- 更新下载进度与速率：设置页检测到新版本后原位切换为下载按钮，经事件实时展示进度；CLI 交互终端同步输出容量、百分比与速率（#17）

### Bug 修复

- 修正卡片与账户页头窄宽布局：多窗口额度区按内容展开并贴近右侧，避免三列指标受固定宽度挤压；账户页头在窄窗口保留横排，设置与添加账户按钮不再过早变为通栏（#17）

### 其他改进

- 新增开发目录分级清理入口 `clean.cmd`：轻量/标准/深度三级，固定白名单与仓库边界校验保护源码与密钥文件，支持 WhatIf 预览，附隔离沙箱契约测试（#17）
- 同步 stable rustfmt 对既有代码的机械格式基线，修复 Windows 与 Ubuntu CI 格式检查阻断（#16）

## [0.2.0] - 2026-08-23

本版本聚焦智谱 GLM Coding Plan 订阅体验：MCP 工具限额独立显示、套餐变体声明、额度重置倒计时全链路贯通；同时修复悬停面板在隐藏托盘场景的系列定位问题，并打磨托盘菜单与卡片排版。

### 新功能

**查询与平台（core）**

- 智谱/Z.ai 多窗口重构：限额条目按 `unit` 字段归类 5 小时/每周窗口，无法识别的条目宁缺毋错（不再错标成周限额）；`TIME_LIMIT` 类型识别为 MCP 工具限额（月度窗口）独立成行，不受套餐变体过滤
- 套餐变体声明：条目新增「套餐变体」字段（自动推断 / 有周限 / 无周限），智谱 v1（无周限）与 v2+（有周限）可显式声明；字段为通用设计，其他平台接入订阅制时可复用
- 额度重置时刻捕获：用量数据新增通用字段 `reset_at`，智谱各窗口（5h/周/MCP）的重置时间填入；余额类平台无重置语义不填，未来订阅制平台接入即通两端展示

**命令行（quota-cli）**

- `query` 表格新增「重置」列：倒计时自适应格式（`21m` / `3h21m` / `4d17h`），`--json` 同步透出重置时刻
- `add` / `edit` 向导对支持套餐变体的平台（智谱系）追加变体问询

**桌面端（quota-desktop）**

- 额度重置倒计时：卡片数值左下角与悬停面板数值下方的小号倒计时（悬停提示「N 后重置」），与 CLI 同格式
- 卡片多窗口主数值区：多窗口条目（智谱 5h/周限/MCP）横向并排逐窗显示（短标签 + 数值 + 倒计时），替代原先只显示首窗口
- 悬停面板压缩布局：垂直空间不足时自动裁剪状态行/用量列表/峰谷/页脚，保留头部与核心数值；面板头部新增关闭按钮
- 标题栏新增 GitHub 仓库链接按钮（明暗主题自适应）
- 暗色主题中性化：表面色系改 `#161616`，滚动条随主题换色（`#2e2e2e`）
- 峰谷时段提示文字按高峰/空闲红绿语义着色

### Bug 修复

- 悬停面板在隐藏托盘（overflow flyout）场景的系列修复：锚点错设为固定角落、被 flyout 遮挡、面板挡住图标本体——改为光标工作区包含性判定 + 图标上方锚定 + 置顶不抢前台 + 垂直避让图标高度
- 切换计价模型后悬停面板出现「无数据」空窗：保存条目后立即补查并广播结果
- 智谱多窗口曾显示两行同名「GLM Coding Plan」无法区分（随多窗口重构修复）
- 安装包与主程序图标未应用品牌主图

### 其他改进

- 托盘右键菜单灰色信息行只挂当前展示条目（与「图标显示」数据源同源），不再随条目数线性膨胀
- 卡片收起态路由标签、模型下拉的超长文字统一单行截断 + 悬停显示全文；多窗口标签字体与「可用余额」样式统一

## [0.1.0] - 2026-08-23

首个发布版本：托盘常驻的多平台 AI 账户余额监视器，CLI 与 GUI 双前端共享同一业务内核（quota-core），凭据全程密文存储。

### 新功能

**查询与平台（core）**

- 8 个预置平台余额查询：DeepSeek、SiliconFlow（国内/国际）、OpenRouter、Kimi（国内/国际）、智谱、Z.ai（GLM Coding Plan 用量，多窗口）（#3 #12）
- 声明式模板自定义查询：以 JSON 描述 URL/请求头/字段映射/算术变换/多窗口，零代码接入任意平台；保存时静态校验，支持真实试查（#3）
- 凭据加密存储：随机 32 字节主密钥存系统凭据库，凭据字段 AES-256-GCM 加密落盘（`v1:` 版本化密文格式），任何文件、日志与错误信息均不落明文（#1 #3）
- 峰谷定价体系：按「周几 + 时间段」判定高峰/空闲，每档三价（缓存命中/未命中/输出，每 MTokens）；DeepSeek 官方价格随版本预置，条目可字段级自定义（留空回退预置）；区分按量计费与订阅积分两种模式；自定义模型库按平台增补模型价格（#7 #9 #12 #13）
- 更新检测基础能力：版本三段比较、GitHub release 解析、节流与每日定时检测（#6）

**命令行（quota-cli）**

- 全套账户管理子命令：`list` / `query`（`--watch` 轮询、`--json` 机器可读输出、并行查询）/ `add`（交互向导，key 星号掩码输入）/ `edit` / `remove` / `setkey` / `natives` / `template test` / `vault status`；退出码三分（0 全成功 / 1 确定性失败 / 2 仅瞬时失败），便于脚本判断是否重试（#1）
- 定价子命令族：`pricing show`（当前峰/谷判定、价格对照表、时段聚合与下次翻转时刻）/ `pricing set` / `pricing clear`（自定义写入与清除回退）、`pricing model list/add/remove`（自定义模型库管理）（#9 #13）
- `quota update`：检测 GitHub release 新版本，可选下载安装包；日常命令启动时 stderr 自动提示新版本（不干扰 stdout 与 `--json`）（#6）
- 中英双语三态：`--lang zh|en|system`，帮助、表格、向导与错误信息全量双语（#4）

**桌面端（quota-desktop）**

- 托盘常驻监视：菜单按条目分组展示余额/已用百分比与更新时间，悬停触发即时刷新（10 秒节流），低额度告警（条目标记 + 图标红点）（#3）
- 托盘圆环图标：额度百分比单弧按阈值变色，不定额余额按「每圈单位」分层叠弧（预设色循环、溢出提示），「图标显示」子菜单可切换数据源条目（#5）
- 托盘峰谷两行：当前峰/谷判定与三档价格，每分钟自动检测时段翻转并刷新（#9）
- 托盘悬停详情面板：悬停托盘图标弹出余额优先浮窗，展示额度窗口与峰谷三档价；账户下拉即时切换圆环数据源，模型下拉持久更新条目计价模型；离开延迟收起、四边自适应定位、明暗主题与双语跟随
- 主窗口：供应商卡片列表（余额优先、hover 展开定价与操作、错误徽标区分瞬时/确定性）、编辑对话框（native 平台下拉 / 模板可视化编辑器带校验与试查 / 峰谷定价结构化编辑：预置模型下拉、一键填充、星期复选窗口编辑）（#3 #11 #14）
- 设置对话框分「常规 / 更新」两页：轮询间隔、告警阈值、开机自启、每圈单位；更新检测开关、每日检测时刻、立即检查、安装包下载（#6 #14）
- 明暗主题三态（跟随系统实时切换）、中英双语三态、自定义标题栏（语言/主题快捷切换，关闭收托盘）（#5）
- 快照持久化：余额快照落盘，重启先渲染上次结果再异步刷新，无空窗期；单实例运行；开机自启（#3）
- Calm Native 视觉系统：明暗设计令牌、统一共享组件、对话框 Escape 关闭与焦点管理（#14）
- 品牌视觉：应用图标与标题栏/悬停面板品牌标志采用「四段额度环 + Q 形拖尾」主图（indigo/mint 双色），全套尺寸经 tauri icon 从主图导出，来源与再生成命令记录于 icons/manifest.json

### 其他改进

- 采用 MIT 许可证（LICENSE，参考项目 cc-switch 同为 MIT）
- 附声明式模板示例库 `examples/templates/`：四个形态的可运行模板（字符串数字单对象 / 双站 `{{baseUrl}}` / 总额与已用展示 / 多窗口展开）及使用说明，均经 `quota template test` 真实试查验证

### 已知限制

- 智谱/Z.ai 使用非官方文档端点（与 cc-switch 同款逆向端点），官方变动可能导致查询失效；智谱团队版请求头暂不支持（#12）
- QuickJS 沙箱脚本查询（计划中的第三类自定义查询）未包含在本版本，复杂平台暂请使用声明式模板
- CLI 中 clap 内置的错误骨架文案（`error:` / `Usage:`）为库英文原文，无法翻译（生态限制）（#4）

<!-- 变更链接 -->
[0.15.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.14.1...v0.15.0
[0.14.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.14.0...v0.14.1
[0.14.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.13.2...v0.14.0
[0.13.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.13.0...v0.13.1
[0.13.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.13.1...v0.13.2
[0.13.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.12.1...v0.13.0
[0.12.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.12.0...v0.12.1
[0.12.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.11.0...v0.12.0
[0.11.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.10.2...v0.11.0
[0.10.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.10.1...v0.10.2
[0.10.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.10.0...v0.10.1
[0.10.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.9.0...v0.10.0
[0.9.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.5...v0.9.0
[0.8.5]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.4...v0.8.5
[0.8.4]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.3...v0.8.4
[0.8.3]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.2...v0.8.3
[0.8.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.1...v0.8.2
[0.1.0]: https://github.com/ONEGAYI/QuotaTray/commits/v0.1.0
[0.2.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.1.0...v0.2.0
[0.3.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.2.0...v0.3.0
[0.3.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.3.0...v0.3.1
[0.3.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.3.1...v0.3.2
[0.4.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.3.2...v0.4.0
[0.4.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.4.0...v0.4.1
[0.4.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.4.1...v0.4.2
[0.4.3]: https://github.com/ONEGAYI/QuotaTray/compare/v0.4.2...v0.4.3
[0.5.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.4.3...v0.5.0
[0.6.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.5.0...v0.6.0
[0.6.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.6.0...v0.6.1
[0.7.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.6.1...v0.7.0
[0.8.0]: https://github.com/ONEGAYI/QuotaTray/compare/v0.7.0...v0.8.0
[0.8.1]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.0...v0.8.1
[0.8.2]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.1...v0.8.2
[0.8.3]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.2...v0.8.3
[0.8.4]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.3...v0.8.4
[0.8.5]: https://github.com/ONEGAYI/QuotaTray/compare/v0.8.4...v0.8.5
