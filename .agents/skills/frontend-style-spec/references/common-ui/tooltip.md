...
- 单行不换行；无边框、无阴影、无箭头；字号继承锚点（根字号 16px）
...
**长文本判定与病根**（2026-08-28，限宽 360px 后复核）：默认气泡 `nowrap`，溢出文字以
`--qt-surface` 色落在页面同色底上**不可见**（观感是"被截断"）。气泡字号继承锚点
（通常根字号 16px），按 16px 估算凡文案可能超过约 42 个拉丁字符或 22 个汉字的挂载点
必须 multiline。已知长文本实例：
...
**字号特例**：模型窗口选择器气泡（`.qt-provider-route` 作用域）固定 12px，对齐卡片
更新时间字号（`qt-provider-meta`），不随锚点继承根字号（2026-08-28 所有者确认）。
