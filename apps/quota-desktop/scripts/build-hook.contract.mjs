...
import { cargoTargetFor, pnpmInvocation, shouldBuildCli } from "./build-hook.mjs";
...
test("Android 构建不编译或打包桌面 CLI sidecar", () => {
  assert.equal(shouldBuildCli("android"), false);
  assert.equal(shouldBuildCli("windows"), true);
  assert.equal(shouldBuildCli("linux"), true);
});
