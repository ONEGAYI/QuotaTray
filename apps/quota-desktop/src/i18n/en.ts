// 英文文案字典：键集合与 zh.ts 完全一致（Record<TextKey, string> 保证编译期完整）。
import type { TextKey } from "./zh";

export const en: Record<TextKey, string> = {
  // ---- 通用 ----
  "common.cancel": "Cancel",
  "common.close": "Close",
  "common.save": "Save",
  "common.saving": "Saving…",

  // ---- 主窗口 ----
  "app.subtitle": "AI account balance monitor",
  "app.add": "+ Add",
  "app.accounts": "Accounts",
  "app.usageStats": "Usage",
  "app.viewTabs": "Main content view",
  "app.accountCount": "{count} account(s)",
  "app.addAccount": "Add account",
  "app.instanceRunning": "QuotaTray is already running; the existing window has been focused",
  "app.settings": "Settings",
  "app.loading": "Loading…",
  "app.configError": "Failed to read config: {msg}",
  "app.emptyTitle": "No provider entries yet",
  "app.emptyHint":
    "Click \"Add\" for a built-in provider, or use a template JSON for any platform",
  "history.title": "History",
  "history.placeholder": "Usage history will be available in a future release",

  // ---- Usage statistics demo ----
  "usage.title": "Usage statistics",
  "usage.provider": "Provider",
  "usage.scope": "Scope",
  "usage.range24h": "24h",
  "usage.range7d": "7d",
  "usage.loadingProviders": "Loading provider configuration…",
  "usage.providersError": "Failed to load providers: {msg}",
  "usage.noProviders": "Add a provider first; history starts after its first successful query.",
  "usage.loadingHistory": "Loading local history…",
  "usage.historyError": "Failed to load history: {msg}",
  "usage.settingsError": "Failed to load usage settings: {msg}",
  "usage.emptyTitle": "No usage history",
  "usage.statusTitle": "Usage statistics",
  "usage.emptyHistory": "No chartable data was recorded for this provider within the selected range. Try again after a successful query.",
  "usage.remainingQuota": "remaining quota",
  "usage.lastSample": "Last sampled {time}",
  "usage.leftAxis": "Left · Quota value",
  "usage.rightAxis": "Right · Percentage",
  "usage.absoluteAxisTitle": "Absolute quota",
  "usage.remainingPercentAxisTitle": "Remaining percent",
  "usage.resetView": "Reset view",
  "usage.chartLabel": "Usage trend for {provider} {scope}",
  "usage.shortGap": "Short gap · dashed bridge",
  "usage.noData": "No samples",
  "usage.missingBuckets": "About {count} sampling buckets missing",
  "usage.noDataHint": "QuotaTray may not have been running; no values were inferred",
  "usage.dragHint": "Drag to pan",
  "usage.zoomHint": "Ctrl + wheel to zoom · double-click to reset",
  "usage.gapLegend": "Hatched areas are long data gaps",
  "usage.addCombination": "Add combination",
  "usage.combinationCount": "{count}/4",
  "usage.addCombinationTitle": "Add comparison",
  "usage.addCombinationDesc": "Choose one provider and model or quota window",
  "usage.modelOrWindow": "Model or quota window",
  "usage.add": "Add",
  "usage.remove": "Remove",
  "usage.autoSaved": "Auto-saved",
  "usage.limitReached": "The four-series limit is reached. Remove one from Focus series first.",
  "usage.noCandidate": "No compatible combinations remain.",
  "usage.unitConflict": "The absolute axis currently uses {unit}",
  "usage.unitConflictHidden": "{count} combination(s) use an absolute unit incompatible with {unit} and are hidden. Remove or adjust them in Focus series.",
  "usage.unavailable": "Currently unavailable",
  "usage.emptySelection": "No comparison combinations added",
  "usage.saveFailed": "Failed to save comparisons: {msg}",
  "usage.resetFocus": "Clear focus",
  "usage.focusHint": "Click a series in Focus series to focus its line",
  "usage.legendTrigger": "Focus series",
  "usage.focusKicker": "FOCUSING",
  "usage.focusEmpty": "No focus",
  "usage.legendRemoveArmed": "Click again to confirm removal",
  "usage.legendHint": "Click a series to focus or unfocus; with a focus, Marker unlocks full measurements",
  "usage.comparisonChartLabel": "Usage comparison chart with {count} series",
  "usage.absoluteValue": "Absolute · {unit}",
  "usage.remainingPercent": "Remaining percent",
  "usage.markerMode": "Marker",
  "usage.markerEmpty": "No markers yet: click \"Marker\", then click the chart to place up to two and measure the span",
  "usage.markerPlaceHint": "Click the chart to place a marker (max 2, snaps to data points); Esc or click again to exit; with two placed, clicking clears both and restarts",
  "usage.markerRemoveOne": "Remove this marker",
  "usage.markerClearAll": "Clear all markers",
  "usage.markerOffscreen": "Marker is outside the current view (pan or zoom to see it)",
  "usage.markerDelta": "Span {span}",
  "usage.markerRate": "Avg. burn {rate}",
  "usage.markerPeakBurn": "Peak burn {rate} ({at})",
  "usage.markerNet": "Net burn {value}",
  "usage.markerNetObservedBurn": "Observed burn {value}",
  "usage.markerNetObservedRecovery": "Observed recovery {value}",
  "usage.markerNetUnobserved": "Unobserved net change {value}",
  "usage.markerNetHint": "Observed burn − observed recovery + unobserved net change = net burn; changes across sampling gaps cannot be split and count toward net only",
  "usage.markerNetDetailToggle": "Toggle net burn breakdown",

  // ---- 供应商卡片 ----
  "card.disabled": "Disabled",
  "card.normal": "Normal",
  "card.snapshot": "Startup snapshot",
  "card.deterministic": "Deterministic failure",
  "card.invalid": "Invalid",
  "card.staleKeep": "Unreachable · showing last value",
  "card.network": "Network issue",
  "card.refresh": "Refresh",
  "card.disable": "Disable",
  "card.enable": "Enable",
  "card.console": "Open console",
  "card.consoleShort": "Console",
  "card.consoleOpenFailed": "Failed to open console",
  "card.edit": "Edit",
  "card.remove": "Remove",
  "card.confirmRemove": "Remove \"{name}\"?",
  "card.disabledNote": "Entry is disabled and not queried",
  "card.invalidPrefix": "Invalid: ",
  "card.noReason": "No reason given",
  "card.noData": "No data yet",
  "card.querying": "Querying…",
  "card.copyError": "Copy error details",
  "card.copied": "Copied",
  "card.copyFailed": "Copy failed",
  "card.windowN": "Window {n}",
  "card.totalQuota": "Total {total}",
  "card.keyConfigured": "API key set",
  "card.keyMissing": "No API key",
  "card.cliCredential": "Credentials from local CLI",
  "card.proxyOn": "Proxy on",
  "card.proxyOnFeedback": "Query proxy enabled",
  "card.proxyOffFeedback": "Query proxy disabled",
  "card.proxyOff": "Use proxy",
  "card.snapshotAt": "Last at {time} (startup snapshot)",
  "card.refreshing": "Refreshing…",
  "card.lastSuccess": "Last success · {time}",
  "card.neverSucceeded": "No successful query yet",
  "card.moreWindows": "+{count} windows",
  "card.resetIn": "Resets in {time}",
  "card.details": "Details",
  "card.collapse": "Collapse",
  "card.periodPeak": "Peak period now",
  "card.periodOffPeak": "Off-peak period now",
  "card.subscriptionPeak": "Standard credit period now",
  "card.subscriptionOffPeak": "Discounted credit period now",
  "card.refreshEvery": "Refresh every {minutes} min",
  "card.retry": "Retry",
  "card.more": "More actions",
  "card.removeTitle": "Remove account",
  "card.switchModel": "Switch pricing model",
  "card.modelChanging": "Switching model…",
  "card.modelChanged": "Switched to {model}",
  "card.modelChangeError": "Model switch failed: {msg}",
  "card.dragHandle": "Drag to reorder",
  "card.dragHandleHint": "Arrow keys also reorder",

  // ---- Tray hover panel ----
  "hover.account": "Account",
  "hover.model": "Model",
  "hover.availableBalance": "Available balance",
  // 摘要位剩余措辞族与 Rust i18n.rs 的 remaining_percent_text/remaining_text
  // 成对用 Left（PR #146 review：全前端统一，不混用 Remaining）
  "hover.remainingQuota": "Left",
  "hover.openMain": "Open main window",
  "hover.close": "Close panel",
  "hover.noEnabled": "No enabled accounts",

  // ---- 添加/编辑对话框 ----
  "edit.titleEdit": "Edit provider",
  "edit.titleAdd": "Add provider",
  "edit.descriptionAdd": "Connect a new AI platform account",
  "edit.tabNative": "Built-in",
  "edit.tabTemplate": "Template",
  "edit.tabScript": "Script",
  "edit.name": "Name",
  "edit.namePlaceholder": "e.g. DeepSeek main",
  "edit.nameRequired": "Name is required",
  "edit.platform": "Platform",
  "edit.platformGroupZhipu": "Zhipu",
  "edit.platformGroupAliyun": "Alibaba Cloud",
  "edit.planVariant": "Plan variant (limit windows)",
  "edit.planVariantAuto": "Auto (infer from response)",
  "edit.planVariantNoWeekly": "No weekly · v1 (5h window only)",
  "edit.planVariantWeekly": "Weekly · v2+ (5h + weekly)",
  "edit.platformPlaceholder": "Select…",
  "edit.nativeRequired": "Select a built-in provider",
  "edit.mobileCliUnsupported": "This provider depends on a desktop CLI login file, which Android cannot read. Choose another provider or manage this entry on desktop.",
  "edit.templateJson": "Template JSON",
  "edit.scriptCode": "Script JS code (request() builds the request, extract(resp) parses the response)",
  "edit.allowInsecure": "Allow non-https endpoints (allowInsecure)",
  "edit.baseUrl": "baseUrl (source of the {{baseUrl}} variable)",
  "edit.consoleUrl": "Console URL",
  "edit.consoleUrlHint": "Card \"Open console\" target; empty = platform preset, filled = override",
  "edit.consoleUrlInvalid": "Console URL must start with http:// or https://",
  "edit.jsonError": "JSON parse error: {msg}",
  "edit.fieldError": "{field}: {reason}",
  "edit.platformOption": "{name} ({id})",
  "edit.templateJsonError": "Failed to parse template JSON",
  "edit.insecureWarn":
    "⚠ allowInsecure is enabled: requests may go over plaintext http and the API key could be intercepted",
  "edit.validate": "Validate",
  "edit.test": "Test",
  "edit.testing": "Testing…",
  "edit.validated": "Validation passed",
  "edit.testOk": "Test succeeded",
  "edit.apiKey": "API key",
  "edit.apiKeyHint": "Credentials are sent only to the backend, encrypted, and never shown in plaintext.",
  "edit.cliCredentialHint":
    "Credentials are read automatically from the locally signed-in official CLI at query time (no API key to fill in)",
  "edit.cliCredentialBridgeHint":
    "Queries hit the official endpoint for subscription usage; traffic bridged to non-official endpoints via tools like CC-Switch is not covered — use a request template to track that endpoint instead",
  "edit.keyConfigured": "Configured (leave blank to keep)",
  "edit.keyMissing": "Not configured",
  "edit.apiKey2": "Second credential (apiKey2)",
  "edit.apiKey2Hint":
    "Used when the template/script references {{apiKey2}} (e.g. the user ID on new-api style sites, sent in the New-Api-User header); encrypted and never echoed back.",
  "edit.key2Optional": "Optional (leave blank to keep)",
  "edit.key2Required": "Required on this platform · AccessKey Secret (leave blank to keep once configured)",
  "edit.guideButton": "Setup guide",
  "edit.guideImageMissing": "Image missing: {alt}",
  "edit.guideDocMissing": "Guide document is missing. Please update to the latest version and retry.",
  "edit.catalogUpdatedHint": "Pricing catalog updated; this editor keeps the data from when it opened. Reopen to use the new data.",
  "edit.primaryMetric": "Primary metric",
  "edit.primaryMetricAuto": "Auto",
  "edit.primaryMetricPercent": "Percent",
  "edit.primaryMetricAmount": "Amount",
  "edit.primaryMetricHint":
    "Preferred metric for summary display; windows lacking its data fall back individually (flagged by test query)",
  // 回退 toast 用无谓语句式点名窗口："{windows} has no …" 在多窗口
  // （"MCP, backup has …"）下主谓不一致（PR #146 review 修复）
  "edit.metricFallbackToAmount": "No percent data for {windows}; showing amount instead",
  "edit.metricFallbackToPercent": "No remaining amount for {windows}; showing percent instead",
  "edit.metricFallbackClose": "Dismiss fallback notice",
  "pricing.retiredTag": "retired",
  "pricing.unknownPrice": "Price unknown",
  "pricing.officialInfo": "Official model information",
  "pricing.verifiedAt": "Verified at",
  "pricing.verificationUnknown": "Verification date unknown",
  "pricing.verificationStale": "Not verified recently",
  "pricing.sourceMissing": "Source not provided",
  "pricing.referenceOnly": "Official model information only; excludes custom prices.",
  "pricing.sourceOpenFailed": "Unable to open source link",
  "pricing.retiredNote": "Model retired: showing last known prices.",

  // ---- Template sub-tabs and presets ----
  "edit.subProvider": "Provider & model",
  "edit.subTemplate": "Template",
  "edit.subScript": "Script",
  "edit.presetLabel": "Preset templates",
  "edit.presetHint": "Click to load this preset into the editor, replacing current content",
  "edit.preset.custom": "Generic",
  "edit.preset.balance": "Single balance",
  "edit.preset.site": "Site via baseUrl",
  "edit.preset.credits": "Total / used",
  "edit.preset.windows": "Multi-window",
  "edit.preset.newapi": "NewAPI gateway",
  "edit.helpTitle": "Template guide",
  "edit.helpLead":
    "A template is declarative JSON describing one balance request and its value paths — no code needed.",
  "edit.helpVarKey": "API key — taken from the credential field on the \"Provider & model\" tab",
  "edit.helpVarUrl": "Site base URL — taken from the baseUrl input on the \"Provider & model\" tab",
  "edit.helpRequest": "Request: method defaults to GET; headers and body are sent as-is",
  "edit.helpExtract":
    "Field mapping: planName / total / used / remaining / unit / isValid / invalidMessage — any subset",
  "edit.helpPath": "Value syntax: a JSONPath (e.g. $.data.balance) or a const (see the example below)",
  "edit.helpTransforms": "Arithmetic: multiply / divide / add / sub / round, applied after extraction",
  "edit.helpWindows":
    "Multi-window: windowsFrom points to the window array in the response; windows maps each one",
  "edit.helpInsecure": "allowInsecure: true allows plaintext http requests (risk of leakage)",
  "edit.helpTip":
    "\"Validate\" is a static check; \"Test\" sends one real request with the current key and baseUrl.",
  "edit.ai.open": "AI debug",
  "edit.ai.title": "AI debugging assistant",
  "edit.ai.description": "Build a sanitized context bundle for an external AI or Agent",
  "edit.ai.safety": "API keys and encrypted credential fields are not collected. Review the preview before sharing because provider-specific secret formats may not be recognized.",
  "edit.ai.docsUrl": "API documentation or gateway guide (optional)",
  "edit.ai.goal": "What should be queried",
  "edit.ai.goalPlaceholder": "e.g. remaining and used quota",
  "edit.ai.responseSample": "Response JSON sample (optional)",
  "edit.ai.responsePlaceholder": "Paste a sanitized API response to improve extraction-path accuracy",
  "edit.ai.responseHint": "Common tokens are scrubbed again. Do not paste API keys, cookies, or full personal data.",
  "edit.ai.preview": "Prompt preview",
  "edit.ai.copyPrompt": "Copy help prompt",
  "edit.ai.savePackage": "Save debug bundle",
  "edit.ai.saving": "Saving…",
  "edit.ai.localAgentPrompt": "Copy for local Agent",
  "edit.ai.copied": "Copied",
  "edit.ai.serviceNote": "QuotaTray only provides debugging services such as schema, static validation, and offline simulation. It does not provide or launch an AI Agent. External Agents may search public documentation themselves.",
  "edit.ai.promptCopied": "Help prompt copied",
  "edit.ai.agentPromptCopied": "Local Agent debugging prompt copied. Paste it directly into an Agent with terminal access.",
  "edit.ai.cliMissing": "Cannot build the local Agent prompt: {error}",
  "edit.ai.copyFailed": "Copy failed. Copy manually from the preview.",
  "edit.ai.saveTitle": "Save QuotaTray AI debug bundle",
  "edit.ai.saved": "Debug bundle saved: {path}",
  "edit.ai.saveFailed": "Failed to save debug bundle: {error}",

  // ---- 自定义标题栏 ----
  "titlebar.github": "GitHub repository",
  "titlebar.language": "Switch language",
  "titlebar.theme": "Switch theme",
  "titlebar.minimize": "Minimize",
  "titlebar.maximize": "Maximize",
  "titlebar.restore": "Restore",
  "titlebar.close": "Close",

  // ---- 设置对话框 ----
  "settings.title": "Settings",
  "settings.description": "App behavior and updates",
  "settings.save": "Save settings",
  "settings.interval": "Refresh interval (min)",
  "settings.intervalTitle": "Auto refresh",
  "settings.intervalHint": "Periodically query all enabled accounts",
  "settings.minutes": "Every {minutes} min",
  "settings.minuteUnit": "min",
  "settings.threshold": "Low-balance alert threshold (remaining %)",
  "settings.thresholdTitle": "Low-balance alert",
  // 判定契约含等号（low_balance_breach / 托盘 ⚠ 用 >=），措辞为
  // "at or below" 而非 "falls below"（PR #146 review 修复）
  "settings.thresholdHint": "Alert when the remaining balance is at or below this percentage",
  "settings.recoveryThresholdTitle": "Balance recovery alert",
  "settings.recoveryThresholdHint": "Alert when quota recovers to this remaining percentage",
  "settings.recoveryThresholdConflict": "Recovery threshold must be higher than the low-balance threshold (both in remaining percentage). Adjust before saving",
  "settings.autostart": "Launch at startup",
  "settings.autostartHint": "Run in the tray after signing in to Windows",
  "settings.notificationsTitle": "System notifications",
  "settings.notificationsHint": "Send update and low-balance alerts as system notifications while the app is in the background",
  "settings.notificationPermissionTitle": "Notification permission",
  "settings.notificationPermissionGranted": "Granted",
  "settings.notificationPermissionMissing": "Not granted; notifications are blocked by the system",
  "settings.notificationRequest": "Request permission",
  "settings.notificationOpenSettings": "Open system settings",
  "settings.notificationOpenSettingsFailed": "Failed to open system settings; enable notifications manually",
  "settings.backgroundRefreshTitle": "Background refresh",
  "settings.backgroundRefreshHint": "Query balances and record history on a schedule while the app is in the background, sending a notification when an alert threshold is reached (~15-minute granularity; may be delayed by system battery savings)",
  "settings.backgroundIntervalTitle": "Background refresh interval",
  "settings.backgroundIntervalHours": "Every {hours} hours",
  "settings.langZh": "中文",
  "settings.langEn": "English",
  "settings.langSystem": "Follow system",
  "settings.mobileLanguageHint": "Choose the display language for the Android interface.",
  "settings.mobileThemeHint": "Choose light, dark, or follow the system.",
  "settings.themeLight": "Light",
  "settings.themeDark": "Dark",
  "settings.themeSystem": "Follow system",
  "settings.ringUnits": "Tray ring units per circle",
  "settings.ringUnitsHint": "Balance represented by each tray ring",
  "settings.tabGeneral": "General",
  "settings.tabUpdate": "Update",
  "settings.tabNetwork": "Network",
  "settings.tabData": "Data",
  "settings.transferTitle": "Cross-machine transfer",
  "settings.transferDescription": "Move providers, credentials, pricing, custom models, and query history",
  "settings.transferWarning": "The package carries a one-time decryption key and is as sensitive as plaintext credentials. Save it only to a trusted location and delete it after migration.",
  "settings.exportTitle": "Export complete configuration",
  "settings.exportHint": "Creates a private QuotaTray transfer package (query history included). The machine master key never leaves the OS credential vault.",
  "settings.exportButton": "Export package",
  "settings.exporting": "Exporting…",
  "settings.exportModalTitle": "Export package",
  "settings.exportModalDescription": "Choose how the package is encrypted",
  "settings.exportModePassword": "Password (recommended)",
  "settings.exportModeConvenient": "Convenient",
  "settings.exportPasswordInfo": "The backup password is never stored. If you forget it, the package cannot be recovered—keep it safe.",
  "settings.exportPasswordLabel": "Backup password (at least 8 characters)",
  "settings.exportPasswordConfirmLabel": "Re-enter the backup password",
  "settings.exportPasswordTooShort": "The password must be at least {min} characters",
  "settings.exportPasswordMismatch": "The two passwords do not match",
  "settings.exportConvenientWarning": "The exported file embeds a one-time decryption key and can recover every account credential (query history included), making it as sensitive as plaintext credentials. Save it only to a trusted location and delete it after migration.",
  "settings.exportModalConfirm": "Export",
  "settings.exportDialogTitle": "Export QuotaTray configuration",
  "settings.exportSuccess": "Configuration exported to: {path}",
  "settings.importTitle": "Import package",
  "settings.importHint": "Import providers, credentials, pricing, and query history from a backup. Merge by default (no local data loss); overwrite optional.",
  "settings.importButton": "Import package",
  "settings.importing": "Importing…",
  "settings.importModalTitle": "Import package",
  "settings.importModalDescription": "Pick a package and choose how it meets your local data",
  "settings.importPickFile": "Choose package file",
  "settings.importPickAnother": "Change",
  "settings.importInspecting": "Identifying file…",
  "settings.importContainerVersion": "Container v{version}",
  "settings.importContainerConvenientMode": "Convenient (no password needed)",
  "settings.importContainerPasswordMode": "Password-protected (backup password required)",
  "settings.importPasswordLabel": "Backup password",
  "settings.importPasswordRequired": "This package is password-protected; enter the backup password",
  "settings.importStrategyLabel": "Import mode",
  "settings.importStrategyMerge": "Merge (recommended)",
  "settings.importStrategyOverwrite": "Overwrite",
  "settings.importMergeHint": "Only adds entries and comparison series missing locally; conflicts keep the local side—nothing local is lost",
  "settings.importOverwriteHint": "Local data becomes exactly the backup contents",
  "settings.importOverwriteWarning": "Overwriting first clears all local providers, credentials, pricing, custom models, comparison series, and query history, then replaces them with the backup. This cannot be undone.",
  "settings.importOverwriteAck": "I understand overwriting removes current local data and cannot be undone",
  "settings.importConfirmOverwrite": "Overwrite import",
  "settings.importConfirmOverwriteCountdown": "Overwrite import ({seconds}s)",
  "settings.importModalConfirm": "Import",
  "settings.importDialogTitle": "Import QuotaTray configuration",
  "settings.importSuccessMerge": "Merge complete: {providersAdded} provider(s) added, {providersSkipped} duplicate(s) skipped; {seriesAdded} comparison series added, {seriesSkipped} skipped.",
  "settings.importSuccessOverwrite": "Overwrite import complete: {providersAdded} provider(s) and {seriesAdded} comparison series imported.",
  "settings.importSuccessMergeProviders": "Merge complete: {providersAdded} provider(s) added, {providersSkipped} duplicate(s) skipped.",
  "settings.importSuccessOverwriteProviders": "Overwrite import complete: {providersAdded} provider(s) imported.",
  "settings.importDegradedHistory": "History data was not written to this device ({reason}); the configuration import itself succeeded — re-importing restores history.",
  "settings.importDegradedUsageComparison": "Usage comparison selections were not written to this device ({reason}); the configuration import itself succeeded — re-importing restores them.",
  "settings.exportDegradedHistory": "The transfer package does not include history data ({reason}); all other configuration was exported.",
  "settings.transferDialogFilter": "QuotaTray configuration transfer",
  "settings.clearTitle": "Clear configuration",
  "settings.clearHint": "Remove all providers, credentials, and local data to get a clean configuration ready to share",
  "settings.clearButton": "Clear data",
  "settings.clearConfirmHeading": "Clear all configuration?",
  "settings.clearConfirmDescription": "This action cannot be undone",
  "settings.clearConfirmWarning": "This permanently removes all providers, encrypted credentials, pricing, and query history.",
  "settings.clearBusy": "Clearing…",
  "settings.clearConfirmButton": "Clear now",
  "settings.clearConfirmCountdown": "Clear now ({seconds}s)",
  "settings.portableTag": "Portable",
  "settings.autostartPortableHint": "Portable builds do not support autostart: the launch entry would point to removable media and leave a dead entry after unplug",
  "settings.downloadPackage": "Download update package",
  "settings.downloadedPortable": "Update package downloaded to: {path}. Quit QuotaTray, then extract the zip over the portable directory (Data/ is unaffected).",
  "settings.downloadedArchive": "Update package downloaded to: {path}. Quit QuotaTray, then extract the zip over the application directory (user data is unaffected).",
  "settings.openDownloadDir": "Open download folder",
  "settings.openDataDirTitle": "Data folder",
  "settings.openDataDirHint": "Open the current data folder (configuration, history, and pricing cache) in the file manager",
  "settings.openDataDirPortableHint": "Open the portable Data/ folder in the file manager; it holds portable.key alongside encrypted credentials—never share it",
  "settings.openLogsDirTitle": "Log folder",
  "settings.openLogsDirHint": "Open the rolling log folder in the file manager (troubleshooting; kept for 7 days)",
  "settings.openDirButton": "Open",
  "settings.openDirFailed": "Failed to open the folder: {error}",

  // ---- Portable first-run confirmation: body keeps only "why + do-not";
  // the full fixed notice folds into the question-mark click-to-expand
  // disclosure (noticeFull matches README.en.md verbatim). ----
  "portable.initTitle": "First run of the portable build",
  "portable.noticeWhy": "The master key lives next to your encrypted credentials in `Data/`, so **the whole folder equals plaintext credentials** — a leak exposes every API key inside.",
  "portable.noticeRule": "Do not upload it to cloud drives, commit it to version control, or share it with others; if it leaks, **rotate every API key it contained immediately**.",
  "portable.noticeFull": "⚠️ **Portable security notice**: the portable build stores the master key that decrypts your credentials in `Data/portable.key`. Although credentials remain AES-GCM encrypted in the configuration, the key and the ciphertext live in the same portable directory, so the entire `Data/` folder must be treated as plaintext credentials. Do not upload it to cloud drives, commit it to version control, or share it with others; if the medium is lost or the folder leaks, rotate every API key it contained immediately.",
  "portable.infoLabel": "View the full security notice",
  "portable.infoTooltip": "Click to expand the details",
  "portable.confirmHint": "Confirming creates the portable master key in Data/portable.key and initializes the data directory; cancelling writes no sensitive file and exits.",
  "portable.confirm": "I understand the risk, create the portable key",
  "portable.confirmCountdown": "I understand the risk, create the portable key ({seconds}s)",
  "portable.cancel": "Cancel and exit",
  "settings.updateEnabledTitle": "Automatic update checks",
  "settings.updateEnabledHint": "Checks for new versions in the background while the app is running",
  "settings.updateProxyPort": "Network proxy port",
  "settings.updateProxyPortTitle": "Network proxy port",
  "settings.updateProxyPortHint": "Route installer downloads and proxied provider queries through an HTTP proxy (e.g. 7897 for Clash); update checks try a direct connection first and fall back to the proxy; leave empty for direct connection",
  "settings.updateProxyHost": "Network proxy host",
  "settings.updateProxyHostTitle": "Network proxy host",
  "settings.updateProxyHostHint": "IP or hostname of the proxy; empty means local (127.0.0.1). On Android, enter your PC's LAN IP to use its proxy (enable Allow LAN in the proxy app)",
  // #133: pointer shown on the Update tab after proxy fields moved to Network
  "settings.proxyMovedHint": "Proxy host and port are configured on the Network page.",
  "settings.proxyMovedOpen": "Go to Network",
  "settings.updateAutoDownloadTitle": "Auto-download installer",
  "settings.updateAutoDownloadHint": "Silently download new versions when detected (installed builds only); you will be asked before installing",
  "settings.currentVersion": "Current version",
  "settings.latestVersion": "Latest version",
  "settings.lastCheck": "Last check: {time}",
  "settings.neverChecked": "Not checked yet",
  "settings.checkNow": "Check now",
  "settings.checking": "Checking…",
  "settings.upToDate": "Up to date",
  "settings.updateCheckFailed": "Update operation failed",
  "settings.catalogAutoUpdateTitle": "Auto-update models & pricing",
  "settings.catalogAutoUpdateHint": "Periodically check pricing catalog updates while running (every 6 hours; retry no sooner than 30 minutes after a failure); manual update still works when off",
  "settings.catalogTitle": "Model & pricing catalog",
  "settings.catalogUpdateNow": "Update now",
  "settings.catalogUpdating": "Updating…",
  "settings.catalogResultUpdated": "Updated to revision {revision}",
  "settings.catalogResultUnchanged": "Up to date (revision {revision})",
  "settings.catalogResultBusy": "Another update is in progress; try again later",
  "settings.catalogResultFailed": "Update failed: {msg}",
  "settings.catalogScheduleOnDesktop": "Auto-update is on: checks automatically about every 6 hours while the app is running; retries no sooner than 30 minutes after a failure.",
  "settings.catalogScheduleOnMobile": "Auto-update is on: checks automatically about every 6 hours while the app is in the foreground (rechecked when you return to the app); retries no sooner than 30 minutes after a failure.",
  "settings.catalogScheduleOff": "Auto-update is off: no automatic network checks; you can still fetch the latest catalog with Update now.",
  "settings.updateAvailable": "Version v{version} is available",
  "settings.updateError": "Check failed: {msg}",
  "settings.download": "Download installer",
  "settings.downloading": "Downloading…",
  "settings.downloaded": "Saved to: {path}",
  "settings.install": "Install update",
  "settings.installConfirmTitle": "Install update",
  "settings.installConfirm": "QuotaTray will quit, install silently in the background and restart automatically. Continue?",
  "settings.installConfirmButton": "Quit & install",
  // ---- Title bar message center ----
  "titlebar.messages": "Messages",
  "msgCenter.empty": "No messages",
  "msgCenter.updateReadyTitle": "Update ready",
  "msgCenter.updateReadyBody": "QuotaTray {version} installer has been downloaded",
  "msgCenter.installNow": "Install now",
  "msgCenter.autoRestartHint": "QuotaTray will briefly quit and restart automatically",
  "msgCenter.installFailed": "Failed to start the installer; retry in Settings · Update",
  "msgCenter.updateAvailableTitle": "Update available",
  "msgCenter.updateAvailableBody": "QuotaTray {version} is available",
  "msgCenter.viewUpdate": "View update",
  "msgCenter.updateGoToHint": "Download and install in Settings · Update",
  "msgCenter.lowBalanceTitle": "Low balance",
  "msgCenter.lowBalanceBody": "{name} has {remaining}% left",
  "msgCenter.balanceRecoveredTitle": "Balance recovered",
  "msgCenter.balanceRecoveredBody": "{name} balance recovered, {remaining}% remaining",
  "settings.manualUrl": "No installer for this version; release page: {url}",
  // ---- Android updates (SAF save + system installer flow) ----
  "settings.apkDialogTitle": "Save update package",
  "settings.apkDialogFilter": "Android package",
  "settings.downloadedApk": "Update package saved ({name}). Tap \"Install\" to hand it to the system installer",
  "settings.installApk": "Install",
  "settings.noInstaller":
    "No system installer available. Open the saved APK from your file manager to install it manually",
  "settings.installConsentHint":
    "No install confirmation shown? Allow this app to install unknown apps first (open the system setting via the link above, then retry), or open the saved APK from your file manager to install it directly",
  "settings.installConsentOpen": "Open system setting",
  "settings.installConsentUnsupported":
    "This system does not provide that settings page (version too old); install from your file manager instead",
  "settings.installConsentFailed":
    "Failed to open the system settings page; install from your file manager instead",
  // Accessible name of the error-detail disclosure icon (hover tooltip disabled on touch)
  "settings.errorDetail": "Error details",
  // ---- 峰谷定价（EditDialog 编辑区块） ----
  "pricing.section": "Peak/off-peak pricing",
  "pricing.unit": "All prices are per 1M tokens",
  "pricing.unitShort": "1M tokens",
  "pricing.summaryPreset": "{model} · {timezone} · {currency}",
  "pricing.summaryCustom": "Custom overrides based on {model}",
  "pricing.summaryOff": "No windows or prices configured",
  "pricing.statusPreset": "Platform preset",
  "pricing.statusCustom": "Customized",
  "pricing.statusOff": "Off",
  "pricing.modePreset": "Use platform preset",
  "pricing.modeModel": "Use model definition",
  "pricing.modeCustom": "Custom overrides",
  "pricing.modeOff": "No pricing",
  "pricing.modeConfigure": "Custom pricing",
  "pricing.presetModel": "Pricing model",
  "pricing.presetDefault": " (default)",
  "pricing.presetNote": "Official preset prices update with the app",
  "pricing.libraryModel": "Custom model",
  "pricing.libraryNote": "From the custom model library in config",
  "pricing.subscriptionShort": "Subscription",
  "pricing.subscriptionPlan": "Subscription credits / quota",
  "pricing.subscriptionHint": "Subscription quota follows platform credit rules; model windows distinguish standard and discounted periods.",
  "pricing.inheritNote": "Fill only what you want to change. Blank fields use the selected model preset.",
  "pricing.clearOverrides": "Clear all overrides",
  "pricing.customModel": "Custom model",
  "pricing.noModel": "No model selected",
  "pricing.windowsTitle": "Peak windows",
  "pricing.windowsHint": "Any matching window is peak; all other times are off-peak.",
  "pricing.schedulePreset": "Use preset",
  "pricing.scheduleCustom": "Customize",
  "pricing.timezone": "UTC offset (minutes)",
  "pricing.timezoneValue": "Currently {value}",
  "pricing.timezoneInherited": "Using preset {value}",
  "pricing.timezoneLocal": "Blank uses the local timezone",
  "pricing.windowCount": "{count} peak window(s)",
  "pricing.timezoneCurrency": "Timezone and currency",
  "pricing.emptyWindows": "No custom windows yet. Times outside added windows use off-peak prices.",
  "pricing.windowN": "Window {n}",
  "pricing.addWindow": "+ Add peak window",
  "pricing.removeWindow": "Remove",
  "pricing.day": "Days",
  "pricing.day.mon": "Mon",
  "pricing.day.tue": "Tue",
  "pricing.day.wed": "Wed",
  "pricing.day.thu": "Thu",
  "pricing.day.fri": "Fri",
  "pricing.day.sat": "Sat",
  "pricing.day.sun": "Sun",
  "pricing.start": "Start",
  "pricing.end": "End",
  "pricing.to": "to",
  "pricing.priceTitle": "Prices",
  "pricing.priceHint": "Blank inputs use the preset price shown below.",
  "pricing.priceHintNoPreset": "Enter peak and off-peak prices; leave fields blank when not applicable.",
  "pricing.currency": "Currency",
  "pricing.modelTag": "Model display name",
  "pricing.customModelPlaceholder": "e.g. flash",
  "pricing.peak": "Peak price",
  "pricing.offPeak": "Off-peak price",
  "pricing.hit": "Cache-hit input",
  "pricing.miss": "Cache-miss input",
  "pricing.out": "Model output",
  "pricing.hitExplain": "Input token price when the provider cache is hit",
  "pricing.missExplain": "Price for new input or tokens that miss the cache",
  "pricing.outExplain": "Price for output tokens generated by the model",
  "pricing.resetTier": "Use preset",
  "pricing.inheritValue": "Preset {value}",
  "pricing.noValue": "Not set",
};
