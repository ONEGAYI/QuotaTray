...
function nearestSample(scope: UsageScope, timestamp: number): UsageSample | null {
  return nearestUsageSample(scope.samples, timestamp, scope.bucketMs);
}
...
export function UsageStatsPage({ providers, providersLoading, providersError, mobile = false }: Props) {
  ...
  // 触屏双指窗口期：拖动以起始快照为基线落盘，期间必须封住其余定位线写入口
  // （垃圾桶清空/按钮清空/第二指放置），否则松手落盘会把刚清空的线整字段写回
  ...
  // 收起聚焦浮层并复位 armed 删除态与陈旧错误：桌面经悬停宽限到期/失焦/Esc 触发（触发钮
  // 的键盘 Enter/Space 也走 toggle 收起），移动端经再次点击药丸或模态遮罩/返回键
  const legendHoverRef = useRef<LegendHoverController | null>(null);
  const closeLegend = useCallback(() => { legendHoverRef.current?.cancelClose(); setLegendOpen(false); setArmedRemoveId(null); setLegendError(null); }, []);
  // 打开浮层统一入口：显式打开意图应作废挂起的宽限收起——否则鼠标移出后的
  // 240ms 内经键盘 Tab/Enter 展开的浮层会被旧倒计时误关
  const openLegend = useCallback(() => { legendHoverRef.current?.cancelClose(); setLegendOpen(true); }, []);
  // 悬停收起宽限：移出触发区不立即收起，给鼠标穿越触发钮与浮层之间空隙留时间；
  // 宽限内回到区域（含浮层本体）即取消。控制器随 effect 创建/销毁成对——
  // StrictMode 开发模式会演练"挂载→卸载→重挂"，渲染期建一次的实例会被演练
  // 卸载的 cleanup 永久 dispose 且不再重建，宽限收起从此失效（浮层关不上）
  useEffect(() => {
    const controller = createLegendHoverController(closeLegend);
  ...
  return <section className="qt-usage-page" aria-label={t("usage.title")}>
    <div className="qt-usage-toolbar"><div className="qt-usage-comparison-actions"><Button icon={Plus} disabled={removePending} onClick={() => setAddDialogOpen(true)}>{t("usage.addCombination")} <span>{t("usage.combinationCount", { count: effectiveSelection.length })}</span></Button>{legendTriggerVisible(legendItems.length) && <div className="qt-usage-legend" onMouseEnter={mobile ? undefined : () => legendHoverRef.current?.cancelClose()} onMouseLeave={mobile ? undefined : (event) => { const active = document.activeElement; if (event.currentTarget.contains(active) && active instanceof Element && active.matches(":focus-visible")) return; legendHoverRef.current?.scheduleClose(); }} onBlur={mobile ? undefined : (event) => { if (event.relatedTarget && !event.currentTarget.contains(event.relatedTarget)) closeLegend(); }}><Button icon={Focus} variant="secondary" className="qt-usage-legend-trigger" aria-haspopup="dialog" aria-expanded={legendOpen} onMouseEnter={mobile ? undefined : openLegend} onFocus={mobile ? undefined : openLegend} onClick={(event) => { if (mobile) { if (legendOpen) closeLegend(); else openLegend(); return; } if (event.detail === 0) { if (legendOpen) setLegendOpen(false); else openLegend(); } }}>{t("usage.legendTrigger")}<span className={`qt-usage-focus-platform ${focusPlatform ? "" : "is-empty"}`} aria-hidden="true" style={focusPlatform ? { "--qt-series-color": SERIES_COLORS[focusPlatform.colorSlot] } as CSSProperties : undefined}><span className="qt-usage-focus-kicker">{t("usage.focusKicker")}</span><span className="qt-usage-focus-main">{focusPlatform ? <><span className="qt-usage-focus-name">{focusPlatform.name}</span><strong className="qt-usage-focus-value">{focusPlatform.value != null ? formatNumber(focusPlatform.value, focusPlatform.metric) : "—"}</strong></> : t("usage.focusEmpty")}</span></span></Button>{legendOpen && !mobile && <div className="qt-usage-legend-popover" role="dialog" aria-label={t("usage.legendTrigger")}>{legendRows}{legendError && <span className="qt-usage-legend-error" role="alert">{legendError}</span>}<span className="qt-usage-legend-hint">{t("usage.legendHint")}</span></div>}</div>}</div><div className="qt-usage-range-switch"><SegmentedControl value={range} onChange={selectRange} compact options={[{ value: "24h", label: t("usage.range24h") }, { value: "7d", label: t("usage.range7d") }]} /></div></div>
